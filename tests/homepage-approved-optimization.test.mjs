...
test("approved homepage uses independent component insets and canonical rhythm", () => {
  assert.match(css, /\.home-screen \.lottery-screen\s*\{[^}]*--layout-page-inline:\s*16px;[^}]*--home-gap-logo-switcher:\s*8px;[^}]*--home-gap-switcher-draw:\s*clamp\(6px,\s*0\.9dvh,\s*8px\);[^}]*--home-gap-draw-status:\s*clamp\(8px,\s*1\.15dvh,\s*11px\);/s);
  assert.match(css, /\.home-screen \.home-layout\s*\{[^}]*--home-gap-status-core:\s*clamp\(10px,\s*1\.35dvh,\s*13px\);[^}]*--home-gap-core-features:\s*clamp\(14px,\s*1\.75dvh,\s*17px\);[^}]*--home-gap-features-nav:\s*clamp\(8px,\s*1\.15dvh,\s*11px\);/s);
