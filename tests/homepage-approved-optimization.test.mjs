...
test("approved homepage uses independent component insets and canonical rhythm", () => {
  ...
  assert.match(css, /\.home-screen \.home-layout\s*\{[^}]*--home-gap-status-core:\s*clamp\(9px,\s*1\.35dvh,\s*12px\);[^}]*--home-gap-core-features:\s*var\(--home-gap-status-core\);[^}]*--home-gap-features-nav:\s*clamp\(8px,\s*1\.15dvh,\s*12px\);/s);
