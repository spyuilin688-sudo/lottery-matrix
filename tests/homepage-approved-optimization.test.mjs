...
test("approved homepage uses independent component insets and canonical rhythm", () => {
  assert.match(css, /\.home-screen \.lottery-screen\s*\{[^}]*--layout-page-inline:\s*16px;[^}]*--home-gap-logo-switcher:\s*clamp\(9px,\s*calc\(1\.15dvh\s*\+\s*1px\),\s*12px\);[^}]*--home-gap-switcher-draw:\s*clamp\(7px,\s*calc\(0\.9dvh\s*\+\s*1px\),\s*9px\);[^}]*--home-gap-draw-status:\s*clamp\(9px,\s*calc\(1\.15dvh\s*\+\s*1px\),\s*12px\);/s);
