...
test("通知頁批次區與列表共用 18px 左右間距並保留正式底部安全距離", () => {
  assert.match(featureAdjustmentsCss, /\.notifications-screen-v2\s*\{[^}]*--notification-bulk-inline:\s*18px;/s);
  assert.doesNotMatch(featureAdjustmentsCss, /\.notifications-screen-v2\s*\{[^}]*--notification-list-inline\s*:/s);
