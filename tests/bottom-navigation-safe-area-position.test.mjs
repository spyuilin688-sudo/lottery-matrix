...
test("Matrix 指南與我的子頁共用正式底部安全距離外加 8px", () => {
...
test("通知頁維持 20px 左右間距並保留正式底部安全距離", () => {
  assert.match(featureAdjustmentsCss, /\.notifications-screen-v2 \.feature-body\s*\{[^}]*padding:\s*0 20px calc\(var\(--layout-bottom-nav-clearance\) \+ 8px\);/s);
...
test("狀態卡外框與 Matrix Core 使用單一 10px 間距來源", () => {
