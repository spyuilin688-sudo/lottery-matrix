...
test("Matrix 指南卡片縮小、數字加粗且分隔線保留", () => {
  ...
  assert.match(guideCss, /\.guide-category-card\s*\{[^}]*min-height:\s*30\.888px;[^}]*padding:\s*5\.616px 11\.232px;[^}]*gap:\s*5\.616px;[^}]*border:\s*1px solid var\(--pwa-frame-tertiary\);[^}]*border-radius:\s*var\(--pwa-frame-radius\);[^}]*font-size:\s*16\.848px;/s);
