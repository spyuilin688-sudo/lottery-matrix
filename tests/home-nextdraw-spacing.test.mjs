...
test('next draw row uses two octagonal reference containers with no parent divider', () => {
  ...
  assert.match(css, /\.next-draw-item\s*\{[\s\S]*?gap:\s*4px;[\s\S]*?padding-inline:\s*clamp\(6px, 2vw, 10px\);[\s\S]*?border:\s*0;[\s\S]*?border-radius:\s*0;[\s\S]*?clip-path:\s*polygon\(/);
