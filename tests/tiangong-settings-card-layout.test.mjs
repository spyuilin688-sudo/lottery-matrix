...
test('天工一般設定與段落設定由兩張同規格卡片各自承載', () => {
  ...
  assert.match(page.slice(0, generalStart), /<LotteryTabs selected=\{lottery\} onChange=\{setLottery\} \/>/);
  assert.doesNotMatch(generalCard, /彩球類型/);
  assert.match(generalCard, /<SectionTitle>天工設定<\/SectionTitle>/);
