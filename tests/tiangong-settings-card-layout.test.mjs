...
test('天工一般設定與段落設定由兩張同規格卡片各自承載', () => {
  ...
  assert.match(generalCard, /<SettingLabelIcon type="period" \/>天工期數/);
  assert.match(generalCard, /<output[^>]*aria-labelledby="tiangong-period-label">五十期<\/output>/);
  assert.doesNotMatch(generalCard, /彩球類型|<LotteryTabs/);
  assert.match(page.slice(0, generalStart), /<LotteryTabs/);
  assert.match(generalCard, /aria-label="天工球位"/);
  assert.doesNotMatch(generalCard, /進階探索設定|探索模式|命中條件/);
  assert.match(stageCard, /data-stage="first"[\s\S]*<SectionTitle>第一段 天工設定<\/SectionTitle>/);
  assert.match(stageCard, /data-stage="first"[\s\S]*aria-label="天工球位"[\s\S]*aria-label="版路類型"/);
  assert.match(stageCard, /data-stage="second"[\s\S]*<SectionTitle>第二段 天工設定<\/SectionTitle>/);
  assert.match(stageCard, /data-stage="second"[\s\S]*aria-label="天工球位"[\s\S]*aria-label="版路類型"/);
