...
test("homepage status frame owns 16px inset, zero top padding and 4px card gaps", () => {
  ...
  assert.match(homeCss, /--home-gap-draw-status:\s*clamp\(4px,\s*calc\(1\.15dvh\s*\+\s*1px\),\s*7px\);/);
  assert.match(homeCss, /--home-gap-status-core:\s*clamp\(4px,\s*1\.35dvh,\s*7px\);/);
});
...
test("approved A+B membership cards place plan and expiry side by side without a divider", () => {
  ...
  assert.equal(finalDeclaration(canonicalFeatureCss, ".subscription-status-content", "grid-template-columns"), "12.2cqw max-content minmax(0, 1fr)");
