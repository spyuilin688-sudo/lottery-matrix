...
test("draw order keeps its raised position, 22px height and compact near-flat inner seam", () => {
...
test("bottom navigation keeps four primary columns while quick settings lives in headers", () => {
  ...
  // d8cad62 reduced only the gear; its 44px touch target remains unchanged.
  assert.equal(finalDeclaration(navCss, ".header-settings-button svg", "width"), "18px");
  assert.equal(finalDeclaration(navCss, ".header-settings-button svg", "height"), "18px");
});
