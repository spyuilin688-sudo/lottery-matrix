...
test("homepage status frame owns 16px viewport inset, 1.5px vertical padding and 1.5px card gaps", () => {
  ...
  assert.equal(finalDeclaration(homeCss, ".home-screen .matrix-status-section", "padding-inline"), "0");
...
test("homepage logo is reduced by 8 percent and five features keep their canonical insets and gaps", () => {
...
test("draw footer reference is composed from two octagonal containers, not parent divider lines", () => {
  ...
  assert.match(item, /clip-path:\s*polygon\(/);
