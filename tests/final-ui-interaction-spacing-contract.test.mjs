...
test("首頁狀態區、Matrix Core 與四大功能使用指定節奏", () => {
  ...
  assert.match(homeCss, /\.home-screen \.home-layout\s*\{[^}]*--home-gap-status-core:\s*clamp\(9px,\s*1\.35dvh,\s*12px\);[^}]*--home-gap-core-features:\s*var\(--home-gap-status-core\);[^}]*--home-gap-features-nav:\s*clamp\(4px,\s*0\.7dvh,\s*8px\);/s);
