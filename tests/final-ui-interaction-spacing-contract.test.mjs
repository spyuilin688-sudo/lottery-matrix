...
test("首頁狀態區、Matrix Core 與五大功能使用指定節奏", () => {
  ...
  assert.match(homeCss, /\.home-screen \.home-layout\s*\{[^}]*--home-gap-status-core:\s*10px;[^}]*--home-gap-core-features:\s*14px;[^}]*--home-gap-features-nav:\s*clamp\(8px,\s*2\.2dvh,\s*20px\);/s);
