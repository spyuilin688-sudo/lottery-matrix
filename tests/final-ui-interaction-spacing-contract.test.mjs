...
test("首頁狀態區、Matrix Core 與四大功能使用指定節奏", () => {
  ...
  assert.match(homeCss, /\.home-screen \.home-layout\s*\{[^}]*--home-gap-status-core:\s*clamp\(4px,\s*1\.35dvh,\s*7px\);[^}]*--home-gap-core-features:\s*clamp\(7px,\s*1\.35dvh,\s*10px\);[^}]*--home-gap-features-nav:\s*clamp\(4px,\s*0\.7dvh,\s*8px\);/s);
...
test("通知選號提醒時間控制項為 21px 且第二列外距 1px", () => {
  assert.match(adjustmentCss, /\.notifications-screen-v2 \.notification-time-select\s*\{[^}]*height:\s*21px;[^}]*min-height:\s*21px;/s);
  assert.match(adjustmentCss, /\.notification-bet-grid \.notification-grid-time-row \+ \.notification-grid-time-row\s*\{[^}]*margin-block-start:\s*1px;/s);
});
...
test("訂閱方案與管理訂閱使用指定外距、卡片尺寸與付款節奏", () => {
  ...
  assert.match(featureCss, /\.plan-card\s*\{[^}]*border:\s*1px solid var\(--pwa-frame-secondary\);/s);
  assert.match(featureCss, /\.panel\s*\{[^}]*border:\s*1px solid var\(--pwa-frame-secondary\);/s);
  assert.doesNotMatch(proPlansLayoutCss, /border(?:-color|-radius)?:/);
