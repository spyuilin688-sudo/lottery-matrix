...
test("Matrix 狀態保留彩種與底部導覽並移除自訂設定入口", async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /自訂觸發條件/ })).toHaveCount(0);
  await expect(page.locator('.matrix-status-screen .product-header__actions')).toHaveCount(0);
  await expect(page.locator('.matrix-status-lottery-switcher')).toBeVisible();
  await expect(page.getByTestId('bottom-navigation').getByRole('button')).toHaveCount(4);
});
