...
test("首頁直接使用專用 Logo SVG，維持原本流式尺寸來源", () => {
  assert.match(source, /logo:\s*`?\$\{HOME_ASSET_BASE\}\/HomeLogo\.svg`?/);
  assert.match(css, /\.home-screen \.home-logo-image\s*\{[^}]*width:\s*75%;/s);
  assert.doesNotMatch(css, /\.home-screen \.home-logo-image\s*\{[^}]*visibility:\s*hidden;/s);
  assert.doesNotMatch(css, /\.home-screen \.brand-header\s*\{[^}]*background:\s*url\(/s);
