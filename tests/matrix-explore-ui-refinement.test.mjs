...
test("Matrix Explore icons, controls, spacing and badges use the refined mobile rules", () => {
  ...
  assert.match(ruleBlock(featureCss, "\\.native-select"), /border:\s*1px solid var\(--pwa-frame-tertiary\)/);
  assert.match(ruleBlock(featureCss, "\\.native-select"), /border-radius:\s*var\(--pwa-frame-radius\)/);
  ...
  // DESIGN.md 2026-09-15: layout stays local, appearance belongs to shared controls.
  assert.doesNotMatch(button, /(?:border|border-color|background|color)\s*:/);
  const appearance = ruleBlock(featureCss, "\\.segmented button");
  assert.match(appearance, /color:\s*var\(--lottery-text-secondary\)/);
  assert.match(appearance, /background:\s*var\(--pwa-control-surface\)/);
