...
test('Explore title card keeps one dedicated background asset and reference-like compact geometry', () => {
  ...
  assert.match(css, /data-product-header="Matrix 探索"[\s\S]*aspect-ratio:\s*6\.65\s*\/\s*1/);
  assert.match(css, /data-product-header="Matrix 探索"[\s\S]*grid-template-columns:\s*40px\s+56px\s+minmax\(0,\s*1fr\)\s+auto/);
  assert.match(css, /data-product-header="Matrix 探索"[\s\S]*column-gap:\s*6px/);
});
