...
test("歷史開獎三欄使用單一正式比例並將前兩欄內容幾何置中", () => {
  assert.match(css, /\.draw-history-row\s*\{[^}]*grid-template-columns:\s*60px 66px minmax\(0, 1fr\)/s);
...
test("歷史開獎表格降低分隔線亮度並分開五球與六加一排列", () => {
  ...
  assert.match(css, /\.draw-history-row \.history-numbers\[data-has-special="true"\] \.history-main-numbers[^}]*gap:\s*2px/s);
});
