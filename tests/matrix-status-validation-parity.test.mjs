...
test('status shares every Explore-specific validation declaration', () => {
  for (const css of [preview, refinements]) {
    for (const match of css.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
      ...
      // DESIGN 2026-09-15 explicitly scopes algorithm summary-divider colors;
      // status retains the shared separator fallback and its existing page background.
      if (/^\.explore-validation-summary-separator,\s*\.matrix-explore-main-screen \.validation-summary-divider$/.test(selectors)) {
