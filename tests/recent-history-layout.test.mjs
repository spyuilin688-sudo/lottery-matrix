...
test("Matrix Explore 近10期三欄採期數窄日期中等號碼最大寬度", () => {
  ...
  assert.match(exploreCss, /\.matrix-explore-main-screen \.history-row:not\(\.history-head\) > :nth-child\(1\)\s*\{[^}]*color:\s*#fff;[^}]*font-size:\s*14px;[^}]*font-weight:\s*800;[^}]*white-space:\s*nowrap;/s);
});
...
test("Matrix Explore 近10期五顆彩球放大，六加一彩球響應式縮放", () => {
  ...
  assert.match(ballCss, /\.matrix-explore-main-screen \.history-panel:is\(\[data-lottery="今彩539"\], \[data-lottery="天天樂"\]\) \.number-ball-component\.history-lottery-ball\s*\{[^}]*--number-ball-size:\s*clamp\(28px, 7\.7vw, 30px\);[^}]*--number-font-size:\s*13px;[^}]*--underline-width:\s*12px;/s);
...
test("Matrix Explore 六加一維持統一列高並在特別號球上方保留間距", () => {
  assert.match(exploreCss, /\.matrix-explore-main-screen \.history-panel:is\(\[data-lottery="六合彩"\], \[data-lottery="大樂透"\]\)\s*\{[^}]*--matrix-history-ball-size:\s*clamp\(20px, 5\.7vw, 22px\);/s);
