...
test("Matrix Explore 近10期三欄採期數窄日期中等號碼最大寬度", () => {
  ...
  assert.match(exploreCss, /\.matrix-explore-main-screen \.history-row:not\(\.history-head\) > :nth-child\(1\)\s*\{[^}]*font-size:\s*clamp\(\.5rem, 2\.2vw, \.5625rem\);[^}]*white-space:\s*nowrap;/s);
});
