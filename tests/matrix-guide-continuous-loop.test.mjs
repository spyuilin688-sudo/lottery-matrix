...
test("Matrix 指南滑動卡片與說明維持指定間距、字級及原生觸控", () => {
  assert.match(css, /\.matrix-guide-screen \.guide-category-strip\s*\{[^}]*margin:\s*0 4px 6px;[^}]*padding:\s*3px 0;[^}]*gap:\s*5px;[^}]*border-top:\s*1px solid[^}]*border-bottom:\s*1px solid[^}]*scroll-padding-inline:\s*20px;[^}]*scroll-snap-type:\s*x proximity;[^}]*-webkit-overflow-scrolling:\s*touch;[^}]*touch-action:\s*pan-x pan-y;/s);
  assert.match(css, /\.matrix-guide-screen \.guide-category-strip \.guide-category-card\s*\{[^}]*flex:\s*0 0 auto;[^}]*width:\s*max-content;[^}]*min-height:\s*30\.888px;[^}]*padding:\s*5px 9px;[^}]*grid-template-columns:\s*auto auto;[^}]*gap:\s*5px;[^}]*white-space:\s*nowrap;[^}]*font-size:\s*14px;[^}]*font-weight:\s*600;/s);
  ...
  assert.match(css, /\.matrix-guide-screen \.guide-category-strip \.guide-category-card\[data-selected="true"\]\s*\{[^}]*font-weight:\s*700;/s);
