...
test('homepage components retain their independent widths and spacing', () => {
  ...
  assert.match(home, /\.home-screen \.home-shortcut-row\s*\{[^}]*width:\s*100%;[^}]*margin-inline:\s*auto;[^}]*padding-inline:\s*var\(--home-feature-inline\);[^}]*column-gap:\s*var\(--home-feature-gap\);/s);
});
