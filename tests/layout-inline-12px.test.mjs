...
test('homepage components retain their independent widths and spacing', () => {
  assert.match(home, /\.home-screen \.lottery-screen\s*\{[^}]*padding:\s*0;/s);
  ...
  assert.match(home, /\.home-screen \.matrix-status-card\s*\{[^}]*aspect-ratio:\s*1\.9;/s);
  assert.match(home, /\.home-screen \.matrix-status-artwork\s*\{[^}]*width:\s*100%;[^}]*height:\s*100%;[^}]*object-fit:\s*contain;/s);
