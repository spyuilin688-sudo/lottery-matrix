...
test('all title cards use the shared eight-pixel content gap without page overrides', () => {
  ...
  assert.match(responsive, /\.draw-history-screen \.feature-body,\s*\.number-reference-screen \.feature-body\s*\{[^}]*padding:\s*0 var\(--tool-page-inline\) calc\(var\(--layout-bottom-nav-clearance\) \+ 8px\)\s*;/s);
  ...
  assert.match(featurePages, /\.feature-screen:not\(\.home-screen\) > \.feature-body\s*\{[^}]*padding-bottom:\s*calc\(var\(--layout-bottom-nav-clearance\) \+ 8px\)/s);
});
