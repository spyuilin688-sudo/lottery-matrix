...
test('calculator uses the page token and homepage core keeps its fitted responsive geometry', () => {
  ...
  assert.ok(ruleBodies(home, /^\.home-screen \.lottery-screen$/).some((body) => /padding:\s*0;/.test(body)));
  assert.ok(ruleBodies(home, /^\.home-screen \.home-bottom-group$/).some((body) => /--home-core-width:\s*calc\(min\(100vw, 390px\) - 28px\);/.test(body)));
