...
test("selected lottery keeps a complete shared octagon frame across homepage and status pages",()=>{assert.match(switcherCss,/\.lottery-card::after\s*\{[^}]*background:\s*var\(--home-octagon-frame\);[^}]*-webkit-mask:\s*none;[^}]*mask:\s*none;/s);assert.doesNotMatch(baseCss,/lottery-card\[data-selected="true"\]::after/);assert.doesNotMatch(visualCss,/lottery-card\[data-selected="true"\]::after/);});
