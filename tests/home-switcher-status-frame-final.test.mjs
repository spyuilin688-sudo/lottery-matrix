...
test("unselected lottery dims its logo while selected lottery restores full opacity",()=>{assert.match(switcherCss,/\.lottery-card\s*\{[^}]*border:\s*1px solid color-mix\(in srgb, var\(--home-frame-gold\) 22%, transparent\);[^}]*background:\s*var\(--lottery-neutral-950\);/s);assert.match(switcherCss,/\.lottery-card\[data-selected="true"\]\s*\{[^}]*border-color:\s*color-mix\(in srgb, var\(--home-frame-bright\) 55%, transparent\);[^}]*background:\s*color-mix\(in srgb, var\(--home-frame-gold\) 6%, var\(--lottery-neutral-950\)\);/s);assert.match(switcherCss,/\.lottery-selector-logo\s*\{[^}]*opacity:\s*\.6;/s);assert.match(switcherCss,/\.lottery-card\[data-selected="true"\] \.lottery-selector-logo\s*\{[^}]*opacity:\s*1;/s);assert.doesNotMatch(switcherCss,/\.lottery-card::(?:before|after)\s*\{/);});
