...
test('通知頁使用 v2 緊密密度且右側動作固定欄對齊', () => {
  ...
  assert.match(adjustmentsCss, /\.notifications-screen-v2\s*\{[^}]*--notification-bulk-inline:\s*18px;/s);
  assert.doesNotMatch(adjustmentsCss, /\.notifications-screen-v2\s*\{[^}]*--notification-list-inline\s*:/s);
