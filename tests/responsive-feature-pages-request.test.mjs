...
const referenceSource = readFileSync(new URL("../src/features/NumberReferencePage.tsx", import.meta.url), "utf8");
...
test('歷史、同星、對照單使用 16px 水平外距並由標題卡提供 8px 垂直間距 [header migration]', () => {
  ...
  assert.match(css, /\.product-header\s*\{[^}]*width:\s*100%;[^}]*padding:\s*0 var\(--layout-page-inline\);[^}]*margin-bottom:\s*var\(--layout-section-gap\);/s);
...
test('號碼對照單標題操作使用共用控制項尺寸、內容寬度區域與自適應文字 [header migration]', () => {
  ...
  assert.match(referenceSource, /headerAction=\{<HeaderSettingsButton expanded=\{queryExpanded\} controls="reference-header-settings"/);
  assert.match(source, /reference-refresh-trigger[\s\S]*刷新/);
  assert.match(referenceSource, /headerSettings=\{\{ id: "reference-header-settings", expanded: queryExpanded, floating: queryFloating, content: queryPanel/);
});

test('對照單設定第一列保留三個選單與 42px 刷新欄', () => {
  assert.match(css, /\.reference-query-panel \.query-selects\.three-cols\s*\{[^}]*grid-template-columns:\s*minmax\(0, \.95fr\) minmax\(0, \.8fr\) minmax\(0, 1\.75fr\) 42px/s);
...
test('通知頁使用 v2 緊密密度且右側動作固定欄對齊', () => {
  ...
  assert.match(adjustmentsCss, /\.notifications-screen-v2 \.feature-body\s*\{[^}]*padding-inline:\s*var\(--notification-list-inline\);[^}]*padding-block-start:\s*0;[^}]*padding-block-end:\s*calc\(var\(--layout-bottom-nav-clearance\) \+ 8px\);/s);
  assert.match(adjustmentsCss, /\.notifications-screen-v2 \.notification-list\s*\{[^}]*margin-inline:\s*0;/s);
  ...
  assert.match(adjustmentsCss, /\.notifications-screen-v2 \.notification-icon,[\s\S]*?width:\s*34px;[^}]*height:\s*34px/s);
