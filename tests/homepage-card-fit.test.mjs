...
test("selected lottery frame uses the same rounded-corner treatment as the lottery artwork", () => {
  assert.match(switcherCss, /\.lottery-card\[data-selected="true"\]\s*\{[\s\S]*?border-radius:\s*12px;/);
});
