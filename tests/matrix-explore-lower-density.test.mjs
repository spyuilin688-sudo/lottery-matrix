...
test('compact lower sections', () => {
  ...
  assert.match(layout, /\.history-row,[\s\S]*?height:\s*50px;[\s\S]*?min-height:\s*50px;[\s\S]*?grid-template-columns:\s*minmax\(0, \.88fr\) minmax\(0, \.82fr\) minmax\(0, 3\.3fr\)/);
...
test('320 360 390 preserve 6+1 room', () => {
  for (const viewport of [320,360,390]) {
    const inner=viewport-34, draw=inner*(3.3/5), ball=Math.max(22,Math.min(viewport*.072,28)), gap=Math.max(2,Math.min(viewport*.0085,3.5));
