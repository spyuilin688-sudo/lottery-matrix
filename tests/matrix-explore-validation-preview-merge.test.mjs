...
test("Matrix Explore keeps the requested validation spacing and typography scoped", () => {
  ...
  assert.match(css, /\.explore-validation-number--source,\s*\.explore-validation-number--step,\s*\.explore-validation-number--hit\s*\{[^}]*height:\s*auto;[^}]*aspect-ratio:\s*auto;[^}]*padding-block:\s*0\.5px;[^}]*padding-inline:\s*1px/s);
