...
test("會員方案卡由正式版面 owner 控制左右間距並顯示跟隨方案的分頁指示", () => {
  ...
  assert.match(layoutCss, /\.pro-plans-screen\s+\.plan-carousel\s*\{[^}]*width:\s*100%;[^}]*margin:\s*0;[^}]*padding:\s*0\s+var\(--pro-plans-plan-inline\)\s+18px;[^}]*scroll-padding-inline:\s*var\(--pro-plans-plan-inline\);[^}]*gap:\s*var\(--pro-plans-plan-inline\);/s);
