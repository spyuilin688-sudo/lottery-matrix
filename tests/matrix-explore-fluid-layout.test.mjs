...
test("Matrix Explore control rows match the compact mobile reference density", () => {
  ...
  assert.match(css, /\.explore-settings \.setting-grid\s*\{[^}]*margin-top:\s*12px/s);
  ...
  assert.match(ruleBlock(readFileSync("src/feature-pages.css", "utf8"), "\\.segmented button"), /border:\s*1px solid var\(--pwa-frame-tertiary\)/);
...
test("Matrix Explore statistics and results use compact target density", () => {
  ...
  assert.match(statsHeading, /margin-bottom:\s*12px/);
