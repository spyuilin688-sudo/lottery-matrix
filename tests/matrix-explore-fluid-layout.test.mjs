...
test("Matrix Explore control rows match the compact mobile reference density", () => {
  assert.match(css, /\.matrix-explore-main-screen \.feature-body\s*\{[^}]*row-gap:\s*\.375rem/s);
  ...
  assert.match(stack, /row-gap:\s*4px/);
...
test("Matrix Explore statistics and results use compact target density", () => {
  assert.match(css, /\.matrix-explore-main-screen \.repeat-stats-panel\s*\{[^}]*padding:\s*\.75rem 3px;/s);
  assert.match(css, /\.matrix-explore-main-screen \.result-panel\s*\{[^}]*padding:\s*\.75rem;/s);
