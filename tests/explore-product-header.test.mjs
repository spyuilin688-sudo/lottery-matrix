...
test("one CSS owner fixes every frame to 68px with single-line fitted titles", async () => {
  ...
  // The merged card's 1px outer border surrounds a 66px inner title: 68px total.
...
test("header alignment stays fixed with absent back controls, actions and smaller titles", async () => {
  ...
  const actionFrame = css.match(/\.product-header__frame\[data-actions="true"\]\s*\{[^}]*\}/s)[0];
  assert.match(actionFrame, /--product-header-action-width:\s*var\(--product-header-settings-action-width, 60px\);/);
  assert.doesNotMatch(actionFrame, /grid-template|grid-area|padding|margin/);
