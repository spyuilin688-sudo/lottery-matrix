...
const data={project:{name:'lottery-matrix'},subscriptions:[{rate_plan:{scope:'zone',id:'free'},price:0}],history:[],covered:false};
...
test('daily integration reads Cloudflare once per endpoint and persists through the existing completion',async()=>{
 ...
 assert.equal(saved.p_results.cloudflare.status,'synced');
 assert.equal(saved.p_results.cloudflare.snapshot.currentAmount,null);
 assert.equal(calls.filter(u=>u.includes('api.cloudflare.com')).length,4);
