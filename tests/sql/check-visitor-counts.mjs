...
try {
  ...
  const file = migrations.find(n => n.endsWith('_matrix_visitor_counts.sql'));
  const secureFile = migrations.find(n => n.endsWith('_secure_visitor_counting.sql'));
  await db.exec(readFileSync(file ? `supabase/migrations/${file}` : 'supabase/visitor-counts.sql', 'utf8'));
  assert.ok(secureFile, 'secure visitor counting migration is required');
  await db.exec(readFileSync(`supabase/migrations/${secureFile}`, 'utf8'));
  ...
  await assert.rejects(db.query('select public.record_matrix_visit($1)',[a]),/permission denied/);
  await assert.rejects(db.query("select public.record_matrix_visit_edge('203.0.113.7')"),/permission denied/);
  ...
  await db.query("select public.record_matrix_visit_edge('203.0.113.7')");
  await db.query("select public.record_matrix_visit_edge('203.0.113.7')");
  await db.query("select public.record_matrix_visit_edge('198.51.100.4')");
  const stats=(await db.query('select public.admin_visitor_stats() result')).rows[0].result;
  assert.equal(stats.totalVisitors,5); assert.equal(stats.todayVisitors,todayBefore+2); assert.equal(stats.monthVisitors,monthBefore+2);
  ...
  const publicAcl=(await db.query("select has_function_privilege('anon','public.record_matrix_visit(text)','execute') allowed")).rows[0];
  const edgeAcl=(await db.query("select has_function_privilege('anon','public.record_matrix_visit_edge(text)','execute') allowed")).rows[0];
  assert.equal(publicAcl.allowed,false);
  assert.equal(edgeAcl.allowed,false);
