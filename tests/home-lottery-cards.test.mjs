...
test('homepage hides the composite switcher image and uses four independent card surfaces', () => {
  ...
  assert.match(css, /\.home-screen \.lottery-switcher-hit-grid\s*\{[^}]*position:\s*relative;[^}]*grid-template-columns:\s*repeat\(4, minmax\(0, 1fr\)\);/s);
  assert.match(css, /\.home-screen \.lottery-switcher-hit-grid\s*\{[^}]*grid-template-columns:\s*repeat\(4, minmax\(0, 1fr\)\);[^}]*gap:\s*6px;/s);
});

test('each lottery button displays its own quarter of the formal Matrixbba artwork', () => {
  assert.match(css, /\.home-screen \.lottery-switcher[^}]*\.lottery-card\s*\{[^}]*background-image:\s*url\("\/assets\/lottery\/status\/Matrixbba\.png"\);/s);
  assert.match(css, /\.home-screen \.lottery-switcher[^}]*\.lottery-card\s*\{[^}]*background-size:\s*calc\(400% \+ 24px\) 100%;/s);
