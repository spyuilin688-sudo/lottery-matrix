...
type StoredConfig = { member_id?: unknown; lottery?: unknown; status?: unknown; config?: unknown };
...
export function createCustomStatusStore(
  ...
  return {
    ...
    async listByLottery(lottery: MatrixLottery): Promise<Array<{
      memberId: string;
      configs: CustomStatusConfig[];
    }>> {
      const path = await endpoint();
      path.searchParams.set('select', 'member_id,status,config');
      path.searchParams.set('lottery', `eq.${lottery}`);
      path.searchParams.set('order', 'member_id.asc,status.asc');
      const rows = await request(path, { method: 'GET' }, 'SUPABASE_CUSTOM_STATUS_READ_FAILED') as StoredConfig[];
      const grouped = new Map<string, CustomStatusConfig[]>();
      for (const row of rows) {
        const memberId = String(row.member_id ?? '').trim();
        if (!memberId) throw new Error('SUPABASE_CUSTOM_STATUS_READ_FAILED');
        const configs = grouped.get(memberId) ?? [];
        configs.push(normalizeCustomStatusConfig(row.config));
        grouped.set(memberId, configs);
      }
      return [...grouped.entries()].map(([memberId, configs]) => ({ memberId, configs }));
    },
