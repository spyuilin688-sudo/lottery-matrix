...
import { nextFantasy5DrawAt } from './draw-schedule';
...
const nextDrawCacheTable = 'next_draw_cache';
...
export async function refreshNextDrawCache(lottery: string) { if (!tableByLottery[lottery]) throw new Error('未知彩種'); const nextDrawAt=lottery==='天天樂'?nextFantasy5DrawAt():await scrapeNextDrawAt(lottery); if(!nextDrawAt) return null; const page=await db.list<NextDrawCacheRecord>(nextDrawCacheTable,{limit:10,filter:{lottery}}); const record={lottery,nextDrawAt,updatedAt:new Date().toISOString()}; const current=page.items[0]; if(current) { const [updated]=await db.update(nextDrawCacheTable,[{id:current.id,record}]); if(!updated) throw new Error('下次開獎時間快取更新失敗'); } else { const [id]=await db.add(nextDrawCacheTable,[record]); if(!id) throw new Error('下次開獎時間快取建立失敗'); } return nextDrawAt; }
export async function getMatrixLatest(lottery: string) { const item = (await getMatrixHistory(lottery, 1))[0] ?? null; if (!item) return null; const nextDrawAt=(await readNextDrawCache(lottery))??(lottery==='天天樂'?nextFantasy5DrawAt():null); return { ...item, nextDrawAt }; }
