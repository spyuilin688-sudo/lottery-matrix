...
export async function getMatrixLatest(lottery: string) { const item = (await getMatrixHistory(lottery, 1))[0] ?? null; if (!item) return null; const nextDrawAt = lottery === '天天樂' ? nextFantasy5DrawAt() : await scrapeNextDrawAt(lottery); return { ...item, nextDrawAt }; }
