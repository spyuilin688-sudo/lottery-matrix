...
type AlgorithmType = '加減' | '合值' | '拖牌';
type Draw = { period: string; drawDate: string; numbers: string[]; sortedNumbers?: string[]; drawOrderNumbers?: string[] | null };
type MatrixRequest = { lottery: Lottery; numberOrder: NumberOrder; lockedPosition: number; lockedNumber: number; referenceOffset?: number; referencePosition?: number; predictionDistance: number; ruleCount: 1 | 2; algorithmType: AlgorithmType };
...
function buildCandidateGroup(history: Draw[], sourceIndex: number, request: MatrixRequest, group: string): CandidateGroup | null {
  ...
  for (const target of prediction.numbers.map(Number)) {
    ...
    if (request.algorithmType === '拖牌') addTypedRule(candidateMap, '拖牌', dragRule, target);
      ...
      if (!(request.algorithmType === '加減' && lockedConditionIsArithmeticReference)) addTypedRule(candidateMap, request.algorithmType, candidateRule(request.algorithmType, base.baseNumber, target, max), target);
...
function validation(groups: CandidateGroup[], rules: string[], request: MatrixRequest) {
  ...
  for (const group of groups) {
    ...
    rows.push({ group: group.group, sourcePeriod: group.source.period, sourceNumbers: orderedNumbers(group.source, request.lottery, request.numberOrder), sourceSortedNumbers: group.source.sortedNumbers ?? group.source.numbers, sourceDrawOrderNumbers: group.source.drawOrderNumbers ?? null, referencePeriod: group.reference.period, baseNumber: group.baseNumber, predictionPeriod: group.prediction.period, predictionNumbers: group.prediction.sortedNumbers ?? group.prediction.numbers, candidateRules, matchedRules, hitNumbers, success });
    if (!success) break;
  }
  return rows;
}
...
export async function runMatrixAutomaticExplore(input: unknown) { const request = parseExploreRequest(input); const newestFirst = await getMatrixHistory(request.lottery, null) as Draw[]; return runMatrixAutomaticExploreWithHistory(input, newestFirst); }

export async function runMatrixAlgorithm(input: unknown) {
  const body = record(input); if (body.explorePeriods !== undefined || body.period !== undefined || body.hitCondition !== undefined || body.exploreRange !== undefined || body.exploreDate !== undefined) return runMatrixAutomaticExplore(input); const request = parseRequest(input); const newestFirst = await getMatrixHistory(request.lottery, null) as Draw[]; return evaluateMatrixAlgorithm(request, newestFirst);
}
