...
export type MatrixDraw = { lottery?: MatrixLottery; period: string; drawDate: string; numbers: string[]; sortedNumbers?: string[]; drawOrderNumbers?: string[] | null };
...
function historyForLottery(lottery: Lottery, history: Draw[]) {
  return history.filter((draw) => (draw.lottery ?? lottery) === lottery);
}
...
function matchingSourceIndexes(request: MatrixRequest, history: Draw[]) {
  return history
    ...
    .filter((item) => (item.draw.lottery ?? request.lottery) === request.lottery && numberAt(
...
function evaluateMatrixAlgorithm(request: MatrixRequest, newestFirst: Draw[]): MatrixEvaluationResult {
  const history = historyForLottery(request.lottery, newestFirst).reverse();
...
export function runMatrixExploreGroupWithHistory(input: MatrixExploreGroupInput, newestFirst: Draw[]) {
  newestFirst = historyForLottery(input.lottery, newestFirst);
