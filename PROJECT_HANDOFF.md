...
- 歷史年份讀取沿用前端共享快取；發布後失效的舊請求不得寫回期別或回傳舊年份，改為共用當前讀取。Railway 的 `summary` 查詢依彩種及資料 revision 共用年份快取；版本確認只共用進行中的請求，確保歷史再次修正而未產生新的牌單訊號時，下一次讀取也能取得正確年份。
- 首頁每次訂閱真正就緒時，以 `/api/matrix/result-revisions` 確認四彩版本；相同版本保留快取，變更或缺少版本基準時清除受影響彩種的公開衍生快取。只傳回不透明版本值，不增加固定輪詢；端點回應使用 `no-store`，失敗保留既有到期重查機制。
- 若版本確認期間收到另一份已變更的首頁資料，不以回應先後判斷版本新舊；仍有效的確認會再取得目前版本，避免忽略較新的發布。
- `/api/matrix/latest-result` 只有在資料與版本確認一致時才附帶 `revisions`。前後端皆相容未附版本的舊回應。
- PNG 牌單清單沿用完整摘要、順序與發布驗證；相同來源／發布版本共用已驗證結果及進行中的工作。快取沿用程序內 128 筆、16 MiB、300 秒上限；資料修正、發布代次或啟用版本變更後重新驗證，缺漏及失敗不保留。
- 本次無資料庫遷移；會員資訊、啟動碼、授權及付款的讀寫契約未修改。部署包含 Cloudflare Pages 前端與 Railway 公開 API。
