-- Allow a trusted confirmed snapshot to correct an already-confirmed period.
-- Reuse the existing provisional-period rebase and matrix_draw_changed trigger so
-- meaningful corrections invalidate stale analysis while identical retries stay no-op.
  ...
  for item in select value from jsonb_array_elements(p_draws) order by value->>'lottery',value->>'draw_date',value->>'period' loop
    ...
    if existing.id is not null and incoming.result_status='preliminary' and existing.result_status='confirmed' then
      ...
      if reconcile_confirmed_period then
        ...
        -- that duplicate identity after later estimates have been freed, then
        -- update the original confirmed period in place so its change trigger
        -- invalidates every dependent analysis artifact/run.
