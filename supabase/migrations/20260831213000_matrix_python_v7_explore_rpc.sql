-- Require Matrix Explore RPCs to read matrix-python-v7 complete artifacts only.
begin;
...
begin
  ...
  where run.lottery = v_lottery
    ...
    and run.analysis_version = run.draw_period || ':matrix-python-v7'
...
begin
  ...
  if v_version <> v_draw || ':matrix-python-v7'
    or not exists (
      select 1
      from public.matrix_analysis_runs as run
      where run.lottery = v_lottery
