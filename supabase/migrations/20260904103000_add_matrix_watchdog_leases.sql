create table if not exists public.matrix_watchdog_leases (
  ...
  constraint matrix_watchdog_leases_runner_length
    check (runner_id is null or char_length(runner_id) between 1 and 200)
);
...
as $$
begin
  if nullif(trim(p_runner_id), '') is null
    or p_ttl_seconds not between 60 and 3600 then
    raise exception 'INVALID_WATCHDOG_LEASE';
  end if;
...
create or replace function public.renew_matrix_watchdog_recovery(
  p_lease_key text,
  p_owner_id text,
  p_runner_id text,
  p_ttl_seconds integer
...
create or replace function public.finish_matrix_watchdog_recovery(
  p_lease_key text,
  p_owner_id text,
  p_runner_id text
)
...
revoke all on function public.begin_matrix_watchdog_recovery(text, text, text, integer) from public;
revoke all on function public.renew_matrix_watchdog_recovery(text, text, text, integer) from public;
revoke all on function public.finish_matrix_watchdog_recovery(text, text, text) from public;
