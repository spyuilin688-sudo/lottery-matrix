...
begin
  ...
  if p_decision = 'confirmed' then
    ...
    set current_plan_id = v_request.plan_id,
        ...
        plan_expires_at = greatest(p_now, coalesce(v_member.plan_expires_at, p_now))
...
revoke all on function public.admin_review_transfer_request(uuid, text, timestamp with time zone, uuid, text)
  from public, anon, authenticated;
grant execute on function public.admin_review_transfer_request(uuid, text, timestamp with time zone, uuid, text)
  to service_role;
