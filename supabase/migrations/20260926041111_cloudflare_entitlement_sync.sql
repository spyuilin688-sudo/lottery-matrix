-- Persist verified Cloudflare limits as partial coverage, never a complete billing sync.
...
begin
  ...
  foreach v_provider in array array['github','railway','cloudflare'] loop
    ...
    if (v_item->>'status') is null or v_item->>'status' not in ('synced','partial','failed') then
      raise exception 'Missing provider status';
    end if;
    if v_item->>'status' in ('synced','partial') then
  ...
  update public.admin_architecture_billing_runs set finished_at=now(),results=p_results,
    ...
    status=case when p_results->'github'->>'status'='synced' and p_results->'railway'->>'status'='synced' and p_results->'cloudflare'->>'status'='synced'
      then 'completed'
      when p_results->'github'->>'status' in ('synced','partial') or p_results->'railway'->>'status' in ('synced','partial') or p_results->'cloudflare'->>'status' in ('synced','partial') then 'partial'
      else 'failed' end
    where id=p_run_id;
  return true;
end;
