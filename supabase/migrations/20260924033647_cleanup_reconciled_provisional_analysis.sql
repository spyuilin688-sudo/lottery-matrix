-- Retire a duplicate provisional period's analysis when its draw is replaced by
-- the same official period already present on a different date. Other ingestion
-- paths retain the existing reconciliation and source-invalidation behavior.
begin;
...
declare
  ...
  reconcile_confirmed_period boolean := false;
begin
  ...
  for item in select value from jsonb_array_elements(p_draws) order by value->>'lottery',value->>'draw_date',value->>'period' loop
    ...
    reconcile_confirmed_period := incoming.result_status='confirmed'
      and period_match.id is not null
      and period_match.result_status='confirmed'
      and existing.id is not null
      and existing.id is distinct from period_match.id
      and existing.result_status='preliminary'
      and existing.draw_date is not distinct from incoming.draw_date;

    if existing.id is not null and incoming.result_status='preliminary' and existing.result_status='confirmed' then
      saved := existing;
    else
      -- A confirmed row for the same period is the canonical identity. It may be
      -- corrected by the service-role ingestion path; the BEFORE UPDATE trigger
      -- invalidates stale analysis when date/numbers actually change.
      if period_match.id is not null and period_match.id is distinct from existing.id then
        if not reconcile_confirmed_period and (
          incoming.result_status<>'confirmed' or period_match.result_status<>'preliminary'
          or incoming.draw_date is null or period_match.draw_date is null
          or period_match.draw_date<=incoming.draw_date
        ) then
      ...
      if incoming.result_status='confirmed' and (
        ...
        -- or correcting a same-date row, preserving every later provisional date.
      ...
      if reconcile_confirmed_period then
        -- The preliminary row represents the same corrected draw date. Remove
        -- its analysis before removing the draw identity. Its children and
        -- active pointers cascade from the run; this follows the same
        -- draw -> run lock order as matrix_draw_changed and fenced completion.
        delete from public.matrix_analysis_runs run
          where run.lottery=existing.lottery and run.draw_period=existing.period;
        delete from public.lottery_draws where id=existing.id;
        -- Update the confirmed row in place to invalidate that period's analysis.
        existing := period_match;
      end if;

      if existing.id is not null then
        -- Repeated source snapshots return the persisted row without an UPDATE,
        -- so they produce neither a new tuple nor duplicate realtime changes.
        -- Source metadata still persists even when the numbers are unchanged.
        saved := existing;
        if (existing.period,existing.draw_date,existing.numbers,existing.sorted_numbers,
            existing.draw_order_numbers,existing.source_id,existing.result_status)
          is distinct from (incoming.period,incoming.draw_date,incoming.numbers,incoming.sorted_numbers,
            incoming.draw_order_numbers,incoming.source_id,incoming.result_status) then
