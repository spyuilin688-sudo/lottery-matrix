...
-- Service-only watchdog view of the same active-version pointers used by the
-- production readers. No worker version number is hard-coded here.
...
begin
  ...
  -- When active slots are incomplete, surface the newest real current-period
  -- attempt. A live lease stays running; failed/stale handling remains in the
  -- existing watchdog evaluator.
