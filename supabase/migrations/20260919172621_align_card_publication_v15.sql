-- Align the publication gate with the deployed worker analysis version.
-- Keep the completed-analysis, confirmed-draw, snapshot and lease checks intact.
  ...
  IF p_manifest->'cards' ? 'draw' THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE run.lottery = p_lottery AND run.draw_period = latest.period
        AND run.analysis_version = latest.period || ':matrix-python-v15-draw'
