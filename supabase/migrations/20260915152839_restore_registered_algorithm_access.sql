...
begin
  ...
  return pg_catalog.jsonb_build_object(
    'canUseSeven', v_free_access or v_paid or v_referrals >= 15
      or (v_uid is not null and nullif(pg_catalog.btrim(v_member.line_user_id), '') is not null and v_dow in (2, 5))
...
revoke execute on function public.matrix_tianyan_list(jsonb) from anon;
revoke execute on function public.matrix_tianyan_validation(jsonb) from anon;
revoke execute on function public.matrix_tiangong_list(jsonb) from anon;
revoke execute on function public.matrix_tiangong_validation(jsonb) from anon;
