...
-- Restrict all automatic Mark Six calendar checks to Tuesday, Thursday and Saturday
-- in Asia/Taipei. The configured times remain unchanged.
...
as $$
  ...
  slots as (
    ...
    where pg_catalog.extract(isodow from local_now.value)::integer in (2,4,6)
  )
  select max(slot_at)
  from slots
  where p_now >= slot_at
...
-- pg_cron is UTC; Taipei is UTC+8 and all requested times remain on the same UTC date.
