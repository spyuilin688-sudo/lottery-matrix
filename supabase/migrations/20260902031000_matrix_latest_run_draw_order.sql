-- Historical analysis backfills may complete after the current draw.
-- Select the latest draw by draw date/period, using completion time only to choose
-- between multiple analysis versions of the same draw.
begin;
...
begin
  ...
  ) || (v_payload - 'lottery' - 'drawPeriod' - 'artifactKinds' - 'artifactCounts');
end;
$$;
...
begin
  ...
  select coalesce(
    pg_catalog.jsonb_agg(
      item order by (item->>'highestStreak')::integer desc, item->>'id'
    ),
    '[]'::jsonb
  ) into v_items
...
begin
  ...
  from pg_catalog.jsonb_array_elements(
    pg_catalog.coalesce(v_payload->'items', '[]'::jsonb)
  ) as item
