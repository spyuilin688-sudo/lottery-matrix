-- Cut Explore list/validation RPCs over to canonical v12 results.
-- Result deletion is intentionally deferred until v12 production rows are verified.
begin;
...
begin
  ...
  where run.lottery = v_lottery
    ...
    and run.analysis_version = run.draw_period || ':matrix-python-v12'
...
begin
  ...
  if v_version <> v_draw || ':matrix-python-v12'
    or not exists (
      select 1
      from public.matrix_analysis_runs as run
      where run.lottery = v_lottery
