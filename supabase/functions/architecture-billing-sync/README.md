...
- Cloudflare: synchronizes project access, account billing-history count and usage coverage. No account-wide history row is treated as a Pages invoice or payment. Empty history and uncovered usage stay unknown, never zero. Four read requests run once in the existing daily job; incomplete history (20 or more entries or a larger total_count) fails closed. Preserve separately verified account data and never infer Pages plan, price, quota or debit date from the zone plan.
