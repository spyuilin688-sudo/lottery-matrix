...
import { createAdminTransferPushHandler, type TransferPushJob } from './handler.ts';
...
function normalizeJob(value: unknown): TransferPushJob {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('INVALID_DISPATCH_RPC_RESULT');
  const row = value as Record<string, unknown>;
  const fields = ['id', 'lease_token', 'subscription_id', 'admin_id', 'transfer_id', 'endpoint', 'p256dh', 'auth_key'] as const;
  for (const field of fields) {
    if (typeof row[field] !== 'string' || !row[field].trim()) throw new Error('INVALID_DISPATCH_RPC_RESULT');
  }
  return Object.fromEntries(fields.map(field => [field, row[field]])) as TransferPushJob;
}
...
Deno.serve(createAdminTransferPushHandler({
  dispatchToken: secret('MATRIX_NOTIFICATION_DISPATCH_TOKEN'),
  async claim() {
    const { data, error } = await supabase.rpc('admin_transfer_push_claim');
    if (error) throw error;
    if (!Array.isArray(data) || data.length > 10) throw new Error('INVALID_DISPATCH_RPC_RESULT');
    return data.map(normalizeJob);
  },
  async eligible(job) {
    const { data, error } = await supabase.rpc('admin_transfer_push_eligible', {
  ...
  async finish(job, outcome, disable) {
    const { data, error } = await supabase.rpc('admin_transfer_push_finish', {
