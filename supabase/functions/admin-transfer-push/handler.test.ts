...
import { createAdminTransferPushHandler, type TransferPushJob, type Dependencies } from './handler';

const job: TransferPushJob = {
  id: 'queue-id', lease_token: 'lease', subscription_id: 'subscription', admin_id: 'admin',
  transfer_id: 'transfer', endpoint: 'https://fcm.googleapis.com/fcm/send/token',
...
function fixture(overrides: Partial<Dependencies> = {}) {
  ...
  return { handler: createAdminTransferPushHandler(dependencies), events, finished, payloads };
}
function request(token: string | null = 'dispatch-secret', method = 'POST') {
  return new Request('https://example.com/functions/v1/admin-transfer-push', {
...
describe('admin transfer push dispatch', () => {
  ...
  it('rechecks eligibility immediately before send and completes generic notification', async () => {
    ...
    expect(f.payloads).toEqual([{ title: '樂彩 Matrix 管理後台', body: '有新的轉帳申請，請登入後台查看。', url: '#transfer-requests', tag: 'admin-transfer-transfer' }]);
  });
