import {sanitizeWatchdogStatus,watchdogFreshness} from '../backend/watchdog-status';
...
export function MatrixWatchdogPanel({detail,now=new Date()}:{detail:unknown;now?:Date}) {
 ...
 const stale=watchdogFreshness(status,now)!=='fresh';
  ...
  <div className="matrixChainGrid">
   {status.reports?.map(report=>{
    ...
    return <article className="statusRow" key={report.lottery} aria-label={`${report.lottery} ${report.drawPeriod??'期數未知'}`}>
     ...
     <p className="statusScope">{report.state==='PASS'?'本次資料鏈檢查未發現異常。':<>{diagnosis?.faultLayer?`故障層：${stageNames[diagnosis.faultLayer]}`:'尚未定位故障層'}。{diagnosis?.rootCause?diagnosis.rootCause.code:'根因尚未確認。'}</>}</p>
