...
import { refreshCrawlerSystemStatus, type RefreshClient } from './system-status';
...
type Result = { refresh?: unknown; recovery?: { lottery: string; status: string } };

export function RailwayOperations({ client, canEdit, confirm, disabled = false, onBusyChange }: {
  client: Pick<RefreshClient, 'get'> & { post(url: string): Promise<{ data: Result }> };
  canEdit: boolean;
  ...
  const [lastTask, setLastTask] = useState<ManualRefreshTask | null>(null);
  const polling = useRef<AbortController | null>(null);
  const locked = useRef(false);
  const editAllowed = useRef(canEdit);
  editAllowed.current = canEdit;
  const mounted = useRef(true);
  useEffect(() => { mounted.current = true; return () => { mounted.current = false; polling.current?.abort(); }; }, []);
  useEffect(() => { if (!canEdit) polling.current?.abort(); }, [canEdit]);
  const run = async (action: Action, requestId?: string) => {
    ...
    try {
      ...
      if (action === 'refresh') {
        ...
        const result = await refreshCrawlerSystemStatus(client, targets[lottery], {
          requestId, signal: polling.current.signal,
          current: () => mounted.current && editAllowed.current,
          onProgress: task => { setLastTask(task); setNotice(task.status === 'accepted' ? `${lottery} 已受理更新，尚未完成。` : task.status === 'running' ? `${lottery} 更新執行中，尚未完成。` : ''); },
        });
        if (mounted.current && editAllowed.current) setNotice(`${lottery} 已手動更新至 ${result.period} 期`);
        return;
      }
      const { data } = await client.post(`/api/system-status/${targets[lottery]}/${action}`);
      let message: string;
      if (data.recovery?.lottery === lottery && ['accepted', 'already-running'].includes(data.recovery.status)) {
      ...
      if (mounted.current && editAllowed.current) setNotice(message);
    } catch (cause) {
      if (mounted.current && editAllowed.current) { setNotice(''); setError(`${cause instanceof Error ? cause.message : `${label}未取得回應`}；請先重新檢查執行狀態，再決定是否重試。`); }
    } finally {
    ...
    <section className='railwayOperations' aria-labelledby='railway-operations-title'>
      <div className='railwayOperationHeader'>
        ...
        <div className='railwayOperationControls'>
          ...
          <select id='railway-operation-lottery' value={lottery} disabled={disabled || Boolean(pending) || !canEdit} onChange={event => { setLottery(event.target.value as Lottery); setNotice(''); setError(''); }}>
      ...
      <div className='railwayOperationRows'>
        <div className='railwayOperationRow'>
          ...
          {lastTask?.lottery === lottery && <button type='button' className='compactButton' disabled={disabled || Boolean(pending) || !canEdit} onClick={() => run('refresh', lastTask.requestId)}>查詢更新狀態</button>}
          </div>
        </div>
        <div className='railwayOperationRow'>
      ...
      <p className='railwayOperationHint'>更新及復原皆為背景工作，「已受理」不代表完成；更新會自動查詢結果，復原請稍後按上方「重新檢查」。已有工作執行時不重複啟動；未取得回應時，先檢查再決定是否重試。</p>
