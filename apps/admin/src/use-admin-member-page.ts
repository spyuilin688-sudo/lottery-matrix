...
export function useAdminMemberPage(table: 'users' | 'subscriptions', revision: number, client: Client) {
  const [query, setQuery] = useState({ page: 1, keyword: '', status: 'all' });
  const [attempt, setAttempt] = useState(0);
  const queryString = new URLSearchParams({ page: String(query.page), keyword: query.keyword, status: query.status }).toString();
