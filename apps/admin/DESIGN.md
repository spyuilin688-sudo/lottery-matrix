...
`src/ArchitectureOverview.tsx` 與 `src/architecture-overview.css` 擁有四平台訂閱總覽。沿用 systemSettings 查看權限與共用 panel／compactButton；桌面雙欄、760px 以下單欄，標籤 76px、內容可換行。每個平台顯示方案、方案費用、續費日期、資料確認時間與另開分頁的管理訂閱連結。

資料取自受保護的 `admin_architecture_subscriptions` 已核實紀錄，僅在開啟頁面時讀取，無輪詢、無平台自動帳務同步。缺項顯示「尚未取得」，不以公開定價、上次付款日或用量週期推算實際費用及續費日。讀取失敗顯示錯誤與重試；StrictMode 重複掛載沿用同次請求，卸載後不更新已關閉的頁面。平台帳務資料不打包於前端，也不記錄信用卡或登入憑證。變更訂閱仍至各平台完成。
