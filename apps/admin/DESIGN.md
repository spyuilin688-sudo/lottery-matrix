...
用戶管理、訂閱管理、登入紀錄、審計日誌保留共用搜尋卡片：第一列放搜尋、狀態／方案與筆數，第二列以 1px 分隔線整理日期與排序。其他管理頁只保留本身必要工具，不重複建立共用搜尋卡片；伺服器端資料讀取、搜尋防抖、預設排序與底部分頁維持既有契約。頁面工具列主要動作高 30px、表單確認動作至少 32px、一般行內操作 28–32px、標題列觸控目標至少 40px；頁面工具列、表單卡片與一般區塊採 8px 節奏。按鈕依文字寬度，不強制手機整列。
...
訂閱管理只列出未到期、啟用中且方案天數為 30／90／365 的成功訂閱會員；月費、季費、年費直接排列在同一張表，不切換清單。會員名稱與 LINE ID／Google ID 分欄顯示，沿用同一筆會員資料。訂閱管理保留共用搜尋卡片，第一列包含搜尋、狀態、方案與筆數，第二列集中日期與排序；用戶管理、登入紀錄、審計日誌沿用相同層級。付款紀錄與轉帳申請使用自己的收合／分頁，不重複第二張搜尋卡片。表格內「調整到期日」與「用戶資訊」按鈕高 24px、11px 字級。付款紀錄與沖銷使用整張原生 details 卡片，預設收合；summary 顯示標題與筆數，展開後才呈現紀錄與表單。每筆紀錄以明確分隔線區隔；已確認狀態使用金色標籤，並與高 24px、10px 字級的「記錄沖銷」按鈕並排於右側，不建立多餘空白列。

主選單的登入紀錄每頁 10 筆，顯示登入時間、登出時間、IP、推估地區與裝置資訊，不顯示「本次在線時間」；IP 後緊接推估地區欄；啟動碼管理每頁 10 筆，換頁時清除本頁勾選，避免跨頁誤複製或誤刪。會員資訊視窗內的登入紀錄仍維持既有每頁 5 筆契約。
