...
一般資料頁不顯示共用的搜尋／狀態／日期／排序卡片，標題後直接進入頁面既有工具列或資料表；伺服器端資料讀取、預設排序與底部分頁仍保留。一般操作 28–32px 高，標題列圖示 40px，按鈕依文字寬度；不強制手機按鈕整列。表單、對話框、通知管理、待辦、系統設定使用相同緊湊節奏。
...
訂閱管理只列出未到期、啟用中且方案天數為 30／90／365 的成功訂閱會員；月費、季費、年費直接排列在同一張表，不切換清單。訂閱、付款紀錄與轉帳申請不顯示共用搜尋／狀態／日期／排序卡片，維持既有伺服器端預設查詢與各自分頁；表格內「調整到期日」與「用戶資訊」按鈕高 24px、11px 字級。付款紀錄與沖銷使用整張原生 details 卡片，預設收合；summary 顯示標題與筆數，展開後才呈現紀錄與表單。每筆紀錄以明確分隔線區隔；已確認狀態使用金色標籤，並與高 24px、10px 字級的「記錄沖銷」按鈕並排於右側，不建立多餘空白列。
