...
代辦工作區最大 680px；新增欄位兩行，留言以單一清單的分隔列呈現，桌面操作靠右、窄版移至內容下方。通知工作區最大 960px；卡片共用標題，收件對象與固定內容桌面雙欄、手機單欄；頭像 32px，發送說明與按鈕並列；發送紀錄每頁 10 筆並沿用共用分頁。共用卡片之間 8px，這三個工作區的動作為 28px 高、12px 字級、依內容寬度。
...
訂閱管理只列出未到期、啟用中且方案天數為 30／90／365 的成功訂閱會員；月費、季費、年費直接排列在同一張表，不切換清單。工具列沿用原生 select，以 104px（窄版 88px）的「全部方案／月費／季費／年費」篩選放在搜尋與筆數之間。付款紀錄與沖銷使用整張原生 details 卡片，預設收合；summary 顯示標題與筆數，展開後才呈現紀錄與表單。記錄沖銷按鈕高 28px、11px 字級、依文字寬度，不保留多餘留白。

主選單的登入紀錄每頁 10 筆，IP 後緊接推估地區欄；啟動碼管理每頁 15 筆，換頁時清除本頁勾選，避免跨頁誤複製或誤刪。會員資訊視窗內的登入紀錄仍維持既有每頁 5 筆契約。
