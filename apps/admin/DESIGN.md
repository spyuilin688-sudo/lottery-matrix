...
「新增管理員」每次開啟都使用空白表單與預設查看人員權限，清除先前編輯對象；取消編輯後新增仍走新增確認與 API。管理員列的編輯／刪除圖示按鈕以動作及帳號提供可存取名稱。會員狀態 inactive、disabled 與停用一致顯示「停用」，提供「啟動」操作。
...
服務分組依 Supabase、GitHub、Railway、TinyFish 排列，省略沒有項目的群組。GitHub 的 workflow state 顯示為「工作流程狀態」，不表示自動排程是否啟用。
...
Railway 原生彩種選單固定 104px 寬、28px 高、12px 字級；兩種操作各自一列，常駐用途及適用時機，按鈕靠右。復原說明明列四彩種（含天天樂）依目前狀態補抓資料或續做分析、背景受理不等於完成，以及重新檢查與避免重複送出的方式。保留既有確認、權限、狀態回饋與 API 行為。
