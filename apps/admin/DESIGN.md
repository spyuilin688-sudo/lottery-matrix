...
資料鏈的前後端共用過期判斷：依既有開獎日曆與檢查節點記錄 nextCheckAt，超過下次節點及既有 8 分鐘容許時間才標示待重新確認；舊紀錄沒有節點時保留 18 分鐘保守限制。正常資料鏈顯示「本次資料鏈檢查未發現異常」，異常或缺證據才顯示故障／根因資訊。診斷證據中的未取得資料、未到執行時間及沒有新開獎列為等待，僅通知或未知結果不能列為完成。版面與實際 Cron 不變。
