...
describe('connection status', () => {
  ...
  it('returns every current API with its location and endpoint plus the four jobs', async () => {
    ...
    expect(result.items).toHaveLength(69);
  ...
  it('uses only non-mutating status probes and never exposes configured secrets', async () => {
    ...
    expect(fetcher.mock.calls.every(([input, init]) => {
      if (init?.method === 'POST') {
        ...
        return ['/rest/v1/rpc/matrix_explore_list', '/rest/v1/rpc/matrix_explore_validation', '/rest/v1/rpc/matrix_tianheng_list', '/rest/v1/rpc/matrix_tianheng_validation', '/rest/v1/rpc/matrix_tianshu_list', '/rest/v1/rpc/matrix_tianshu_validation'].includes(path)
