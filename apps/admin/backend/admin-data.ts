...
const definitions: Record<string, TableDefinition> = {
  users: {
    ...
    map: (row) => ({ id: String(row.id), authUserId: row.auth_user_id, lineDisplayName: row.line_display_name, registeredAt: row.registered_at, currentPlanId: row.current_plan_id, planStartedAt: row.plan_started_at, planExpiresAt: row.plan_expires_at, isLifetime: row.is_lifetime, autoRenew: row.auto_renew, status: row.status, referralCode: row.referral_code, invitationCode: row.invitation_code, planName: (row.current_plan as Row | null)?.name ?? null, lastOnlineAt: row.last_online_at }),
  },
  subscriptions: {
    ...
    map: (row) => {
      ...
      return { id: String(row.id), authUserId: row.auth_user_id, lineDisplayName: row.line_display_name, registeredAt: row.registered_at, currentPlanId: row.current_plan_id, planName: plan?.name ?? null, planPrice: plan?.price ?? null, planDurationDays: plan?.duration_days ?? null, planStartedAt: row.plan_started_at, planExpiresAt: row.plan_expires_at, isLifetime: row.is_lifetime, autoRenew: row.auto_renew, status: row.status, referralCode: row.referral_code, invitationCode: row.invitation_code, lastOnlineAt: row.last_online_at };
    },
  },
  loginRecords: {
    ...
    map: (row) => ({ id: String(row.id), adminId: row.admin_id, account: row.account, loginAt: row.login_at, logoutAt: row.logout_at, onlineMinutes: row.online_minutes, ip: row.ip, device: row.device }),
  },
  subscriptionRecords: {
    path: '/rest/v1/payments?select=id,member_id,plan_id,amount,paid_at,status&order=paid_at.desc.nullslast&limit=200',
    map: (row) => ({ id: String(row.id), memberId: row.member_id, planId: row.plan_id, amount: row.amount, paidAt: row.paid_at, status: row.status }),
  },
  auditLogs: {
    ...
    map: (row) => ({ id: String(row.id), operationTime: row.operation_time, adminId: row.admin_id, admin: row.admin, operationType: row.operation_type, targetTable: row.target_table, targetId: row.target_id, content: row.content, beforeData: row.before_data, afterData: row.after_data, ip: row.ip, device: row.device }),
  },
  admins: {
    ...
    map: (row) => ({ id: String(row.id), account: row.account, name: row.name, role: row.role, status: row.status, permissions: { view: Boolean(row.can_view), add: Boolean(row.can_add), edit: Boolean(row.can_edit), delete: Boolean(row.can_delete) }, lastLoginAt: row.last_login_at, createdAt: row.created_at }),
  },
  activationCodes: {
    ...
    map: (row) => ({ id: String(row.id), batchId: row.batch_id, code: row.code, durationType: row.duration_type, createdAt: row.created_at, expiresAt: row.expires_at, redeemedByMemberId: row.redeemed_by_member_id, redeemedAt: row.redeemed_at, status: row.status }),
  },
  plans: {
    path: '/rest/v1/plans?select=id,name,price,duration_days&order=duration_days.asc',
    map: (row) => ({ id: String(row.id), name: row.name, price: row.price, durationDays: row.duration_days }),
  },
  transferRequests: {
    ...
    map: (row) => ({ id: String(row.id), memberId: row.member_id, lineDisplayName: (row.member as Row | null)?.line_display_name ?? null, planId: row.plan_id, planName: (row.plan as Row | null)?.name ?? null, amount: row.amount, transferredAt: row.transferred_at, accountLastFive: row.account_last_five, submittedAt: row.submitted_at, status: row.status }),
  },
};
...
async function listDashboardPayments(api: Requester, resetFilter: string) {
  ...
  for (let offset = 0; ; offset += dashboardPaymentPageSize) {
    const page = await api.request<Row[]>(`/rest/v1/payments?select=id,amount,paid_at,status&status=eq.confirmed${resetFilter}&order=paid_at.asc,id.asc&limit=${dashboardPaymentPageSize}&offset=${offset}`);
    rows.push(...page);
...
export async function getDashboard(api: Requester, currentDate = new Date()) {
  ...
  const storedResetAt = typeof settings[0]?.reset_at === 'string' ? new Date(settings[0].reset_at) : null;
  const resetTime = storedResetAt && Number.isFinite(storedResetAt.getTime()) ? storedResetAt.getTime() : null;
  const resetFilter = resetTime === null ? '' : `&paid_at=gte.${encodeURIComponent(new Date(resetTime).toISOString())}`;
  ...
  const payments = paymentRows.filter((row) => row.status === 'confirmed' && typeof row.paid_at === 'string' && (resetTime === null || new Date(row.paid_at).getTime() >= resetTime)).map((row) => ({ amount: Number(row.amount ?? 0), paidAt: String(row.paid_at) }));
  ...
  const sum = (predicate: (payment: { amount: number; paidAt: string }) => boolean) => payments.filter(predicate).reduce((total, payment) => total + payment.amount, 0);
  const expiring = members.filter((member) => {
    if (typeof member.plan_expires_at !== 'string') return false;
    const remaining = new Date(member.plan_expires_at).getTime() - currentDate.getTime();
  ...
  return {
    ...
    quarterRevenue: sum((payment) => { const paidAt = new Date(payment.paidAt); return paidAt.getUTCFullYear() === currentDate.getUTCFullYear() && Math.floor(paidAt.getUTCMonth() / 3) === quarter; }),
...
function validateAdminInput(input: AdminAccountInput) {
  ...
  const permissions = input.role === '超級管理員' ? { can_view: true, can_add: true, can_edit: true, can_delete: true } : input.role === '營運管理員' ? { can_view: true, can_add: true, can_edit: true, can_delete: false } : { can_view: true, can_add: false, can_edit: false, can_delete: false };
  return { account: input.account.trim().toLowerCase(), name: input.name.trim(), role: input.role, status: input.status, ...permissions, ...(input.password_salt && input.password_hash ? { password_salt: input.password_salt, password_hash: input.password_hash } : {}) };
}
...
export function createAdminData(transport: WriteTransport) {
  async function protectLastEnabledSuper(before: Row, next?: { role: string; status: string }) {
    ...
    const enabled = await transport.selectRows<Row>('admin_accounts', `select=id&role=eq.${encodeURIComponent('超級管理員')}&status=eq.${encodeURIComponent('啟用')}`);
    if (!enabled.some((row) => String(row.id) !== String(before.id))) throw new AdminDataError('系統必須保留至少一位啟用中的超級管理員');
  }
  async function writeAudit(entry: { actor: AdminActor; operationType: string; targetTable: string; targetId?: string | null; content?: string | null; beforeData?: unknown; afterData?: unknown; ip?: string | null; device?: string | null }) {
    if (entry.actor.role === '超級管理員') return;
    await transport.insertRows('audit_logs', [{ admin_id: entry.actor.id, admin: entry.actor.name || entry.actor.account, operation_type: entry.operationType, target_table: entry.targetTable, target_id: entry.targetId ?? null, content: entry.content ?? null, before_data: entry.beforeData ?? null, after_data: entry.afterData ?? null, ip: entry.ip ?? null, device: entry.device ?? null }]);
  }
  async function createAdminAccount(input: AdminAccountInput, actor: AdminActor) {
    ...
    const [created] = await transport.insertRows<Row>('admin_accounts', [validateAdminInput(input)]);
    if (!created) throw new AdminDataError('建立管理員失敗', 500);
    await writeAudit({ actor, operationType: '新增', targetTable: 'admin_accounts', targetId: String(created.id), content: '新增管理員帳號', afterData: safeAdminAuditRow(created) });
  ...
  async function updateAdminAccount(id: string, input: AdminAccountInput, actor: AdminActor) {
    ...
    await writeAudit({ actor, operationType: '修改', targetTable: 'admin_accounts', targetId: id, content: '修改管理員帳號', beforeData: safeAdminAuditRow(before), afterData: safeAdminAuditRow(updated) });
  ...
  async function updateMemberStatus(id: string, status: string, actor: AdminActor) {
    ...
    return transport.supabaseRequest<Row>('rpc/admin_set_member_status', { method: 'POST', body: JSON.stringify({ p_member_id: id, p_status: status, p_actor_id: actor.id, p_actor_name: actor.name || actor.account }) });
  }
  async function updateSubscription(id: string, input: SubscriptionAction, actor: AdminActor, currentDate = new Date()) {
    if (!['activate', 'renew', 'cancel', 'adjustExpiry', 'lifetime'].includes(input.action)) throw new AdminDataError('訂閱操作不正確');
    const planId = String(input.planId ?? '').trim() || null;
    if ((input.action === 'activate' || input.action === 'renew') && !planId) throw new AdminDataError('訂閱方案必填');
    ...
    return transport.supabaseRequest<Row>('rpc/admin_update_subscription', { method: 'POST', body: JSON.stringify({ p_member_id: id, p_action: input.action, p_plan_id: planId, p_expires_at: expiresAt, p_now: currentDate.toISOString(), p_actor_id: actor.id, p_actor_name: actor.name || actor.account }) });
  }
  async function reviewTransferRequest(id: string, decision: string, actor: AdminActor, currentDate = new Date()) {
    if (!['confirmed', 'rejected'].includes(decision)) throw new AdminDataError('審核結果不正確');
    return transport.supabaseRequest<Row>('rpc/admin_review_transfer_request', { method: 'POST', body: JSON.stringify({ p_transfer_id: id, p_decision: decision, p_now: currentDate.toISOString(), p_actor_id: actor.id, p_actor_name: actor.name || actor.account }) });
  }
  async function resetRevenue(actor: AdminActor) {
    if (actor.role !== '超級管理員') throw new AdminDataError('僅超級管理員可重設收入', 403);
    const [saved] = await transport.supabaseRequest<Row[]>('rpc/admin_reset_revenue_baseline', { method: 'POST', body: JSON.stringify({}) });
    const savedResetAt = typeof saved?.reset_at === 'string' ? new Date(saved.reset_at) : null;
    if (!savedResetAt || !Number.isFinite(savedResetAt.getTime())) throw new AdminDataError('收入重設失敗', 500);
  ...
  async function generateActivationCodeBatch(durationType: string, actor: AdminActor) {
    ...
    const rows = await transport.supabaseRequest<Row[]>('rpc/generate_activation_code_batch', { method: 'POST', body: JSON.stringify({ p_duration_type: durationType }) });
    const batchId = rows[0]?.batch_id;
    if (rows.length !== 10 || !batchId || rows.some((row) => row.batch_id !== batchId)) throw new AdminDataError('啟動碼批次建立失敗', 500);
    await writeAudit({ actor, operationType: '批次新增', targetTable: 'activation_codes', targetId: String(batchId), content: '批次建立 10 組啟動碼', afterData: { batchId, durationType, count: 10 } });
    return { batchId: String(batchId), count: 10 };
  }
  return { writeAudit, createAdminAccount, updateAdminAccount, updateOwnAdminName, updateMemberStatus, updateSubscription, reviewTransferRequest, resetRevenue, deleteAdminAccount, deleteActivationCode, generateActivationCodeBatch };
}
