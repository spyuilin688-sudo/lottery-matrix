...
export function createConnectionStatus(dependencies: Dependencies) {
  ...
  const createSharedChecks = () => {
    ...
    const openApiPaths = memoizePromise(async () => {
      ...
      const response = await fetcher(`${current.url}/rest/v1/`, { cache: 'no-store', redirect: 'error', headers: { apikey: current.serviceRoleKey } });
  ...
  const runDefinition = async (definition: ApiStatusDefinition, shared: ReturnType<typeof createSharedChecks>): Promise<ConnectionStatusItem> => {
    ...
    try {
      ...
      if (definition.id === 'admin-api') {
        const response = await fetcher(`${adminUrl}${definition.endpoint}`, { cache: 'no-store', redirect: 'error' });
        ...
        const response = await fetcher(`${current.url}${definition.endpoint}`, { cache: 'no-store', redirect: 'error', headers: { apikey: current.serviceRoleKey } });
        if (!response.ok) throw new Error('AUTH_UNAVAILABLE');
        detail = { status: response.status };
      } else if (definition.id === 'matrix-status-function') {
        const current = await shared.config();
        const response = await fetcher(`${current.url}${definition.endpoint}`, { method: 'OPTIONS', cache: 'no-store', redirect: 'error', headers: { apikey: current.serviceRoleKey } });
      ...
      return { ...base, ok: true, checkedAt: now().toISOString(), responseMs: Math.max(0, now().getTime() - started), detail };
    } catch {
      const workerStatus = definition.location === 'Railway' ? await shared.worker().catch(() => undefined) : undefined;
      return { ...base, ok: false, checkedAt: now().toISOString(), responseMs: Math.max(0, now().getTime() - started), error: safeErrorFor(definition, workerStatus) };
    }
  };
  return {
    async get() {
      ...
      let jobRows: Row[] = [];
      try {
        jobRows = await dependencies.supabase.selectRows<Row>('system_job_status', 'select=job_name,lottery,status,started_at,finished_at,updated_at,error&order=updated_at.desc');
      } catch { jobRows = []; }
