...
export type ApiStatusDefinition = {
  ...
  description: string;
  rpcAccess?: 'public-read' | 'member-read' | 'operation';
};
...
const supabaseRpcDefinitions = [
  ...
  ['matrix_manual_refresh_claim', '受理手動更新', '手動更新', '建立或取得單一彩種正在執行的更新工作。'],
  ['matrix_manual_refresh_status', '查詢手動更新', '手動更新', '依工作編號查詢更新狀態。'],
  ['matrix_manual_refresh_update', '記錄手動更新結果', '手動更新', '由背景工作記錄執行結果。'],
...
const supabaseRpcInventory: ApiStatusDefinition[] = supabaseRpcDefinitions.map(
  ([rpc, name, group, description]) => ({
    ...
    checkMode: 'registry',
    rpcAccess: publicReadRpcs.has(rpc) ? 'public-read' : memberReadRpcs.has(rpc) ? 'member-read' : 'operation',
    description,
  }),
);
...
export const apiStatusInventory: readonly ApiStatusDefinition[] = [
  ...
  { id: 'notification-calendar', name: '六合彩開獎日曆', group: '通知', location: 'Supabase', endpoint: '/rest/v1/rpc/notification_draw_calendar_status', checkMode: 'service', description: '依香港賽馬會官方日期決定選號提醒；週二、四、六固定檢查，週日保險檢查，週一、三、五 23:00 補充檢查，日期待確認時暫停提醒。' },
  { id: 'native-notification-dispatch', name: '通知派送架構', group: '通知', location: 'Supabase', endpoint: '/rest/v1/rpc/admin_notification_delivery_health', checkMode: 'service', description: '驗證事件觸發、動態 Recovery、每小時保底，以及 Web／Native／管理員通知佇列與正式派送紀錄；不發送測試通知。' },
