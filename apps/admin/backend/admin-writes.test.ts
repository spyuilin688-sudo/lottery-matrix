...
describe('authorized Supabase writes', () => {
  it('updates only the signed-in administrator name and writes an audit record', async () => {
    const updateRows = vi.fn(async () => [{
      id: 'admin-1', account: 'admin@example.com', name: '新的名稱', role: '查看人員', status: '啟用', can_view: true, can_add: false, can_edit: false, can_delete: false,
    }]);
    const insertRows = vi.fn(async () => [{ id: 'audit-1' }]);
    const data = createAdminData({
      insertRows,
      selectRows: vi.fn(async () => [{ id: 'admin-1', account: 'admin@example.com', name: '管理員', password_salt: 'secret-salt', password_hash: 'secret-hash' }]),
      updateRows,
      deleteRows: vi.fn(async () => []),
      supabaseRequest: vi.fn(async () => []),
    });

    await expect(data.updateOwnAdminName('新的名稱', actor)).resolves.toMatchObject({ id: 'admin-1', account: 'admin@example.com', name: '新的名稱' });
    expect(updateRows).toHaveBeenCalledWith('admin_accounts', 'id=eq.admin-1', { name: '新的名稱' });
    expect(insertRows).toHaveBeenCalledWith('audit_logs', [expect.objectContaining({ admin_id: 'admin-1', operation_type: '修改', target_id: 'admin-1', content: '修改本人管理員名稱' })]);
    const audit = insertRows.mock.calls[0][1][0] as Record<string, unknown>;
    expect(JSON.stringify(audit)).not.toContain('secret-hash');
    expect(JSON.stringify(audit)).not.toContain('secret-salt');
  });

  it('rejects an empty own name before writing to Supabase', async () => {
    const updateRows = vi.fn();
    const data = createAdminData({ insertRows: vi.fn(), selectRows: vi.fn(), updateRows, deleteRows: vi.fn(), supabaseRequest: vi.fn() });
    await expect(data.updateOwnAdminName('   ', actor)).rejects.toMatchObject({ statusCode: 400 });
  ...
  it('writes one audit record only after a successful mutation', async () => {
    const calls: string[] = [];
    const data = createAdminData({
      insertRows: vi.fn(async (table: string) => { calls.push(`insert:${table}`); return table === 'admin_accounts' ? [{ id: 'new', ...input, last_login_at: null, created_at: '2026-08-21T00:00:00Z' }] : [{ id: 'audit' }]; }),
      selectRows: vi.fn(async () => []), updateRows: vi.fn(async () => []), deleteRows: vi.fn(async () => []), supabaseRequest: vi.fn(async () => []),
    });
  ...
  it('does not write audit records for a super administrator', async () => {
    const insertRows = vi.fn(async (table: string) => table === 'admin_accounts' ? [{ id: 'new', ...input, last_login_at: null, created_at: '2026-08-21T00:00:00Z' }] : [{ id: 'audit' }]);
    const data = createAdminData({ insertRows, selectRows: vi.fn(async () => []), updateRows: vi.fn(async () => []), deleteRows: vi.fn(async () => []), supabaseRequest: vi.fn(async () => []) });
    ...
    expect(insertRows).not.toHaveBeenCalledWith('audit_logs', expect.anything());
  });
  ...
  it('creates exactly ten activation codes through the database RPC', async () => {
    const rpc = vi.fn(async () => Array.from({ length: 10 }, (_, index) => ({ id: String(index + 1), batch_id: 'batch-1' })));
    const data = createAdminData({ supabaseRequest: rpc, insertRows: vi.fn(async () => [{ id: 'audit-1' }]), selectRows: vi.fn(async () => []), updateRows: vi.fn(async () => []), deleteRows: vi.fn(async () => []) });
    await expect(data.generateActivationCodeBatch('30_days', actor)).resolves.toEqual({ batchId: 'batch-1', count: 10 });
    expect(rpc).toHaveBeenCalledWith('rpc/generate_activation_code_batch', { method: 'POST', body: JSON.stringify({ p_duration_type: '30_days' }) });
  });
  ...
  it('deletes the selected activation code and records the deleted value', async () => {
    const deleteRows = vi.fn(); const insertRows = vi.fn(); const selectRows = vi.fn(); const supabaseRequest = vi.fn(async () => ({ deleted: true }));
    const data = createAdminData({ insertRows, selectRows, updateRows: vi.fn(), deleteRows, supabaseRequest });
    await expect(data.deleteActivationCode('code-1', actor)).resolves.toEqual({ deleted: true });
    expect(supabaseRequest).toHaveBeenCalledWith('rpc/admin_delete_activation_code', { method: 'POST', body: JSON.stringify({ p_code_id: 'code-1', p_actor_id: 'admin-1', p_actor_name: '管理員' }) });
    expect(selectRows).not.toHaveBeenCalled(); expect(deleteRows).not.toHaveBeenCalled(); expect(insertRows).not.toHaveBeenCalled();
  });

  it('does not fall back to a non-transactional activation-code delete when the RPC fails', async () => {
    const deleteRows = vi.fn(); const insertRows = vi.fn();
    const data = createAdminData({ insertRows, selectRows: vi.fn(), updateRows: vi.fn(), deleteRows, supabaseRequest: vi.fn(async () => { throw new Error('audit failed'); }) });
    await expect(data.deleteActivationCode('code-1', actor)).rejects.toThrow('audit failed');
    expect(deleteRows).not.toHaveBeenCalled(); expect(insertRows).not.toHaveBeenCalled();
  });

  it('rejects deleting the signed-in administrator', async () => {
    const deleteRows = vi.fn();
    const data = createAdminData({ insertRows: vi.fn(), selectRows: vi.fn(), updateRows: vi.fn(), deleteRows, supabaseRequest: vi.fn() });
    await expect(data.deleteAdminAccount('admin-1', actor)).rejects.toMatchObject({ statusCode: 400 });
    expect(deleteRows).not.toHaveBeenCalled();
  });

  it('keeps the last enabled super administrator during deletion', async () => {
    const deleteRows = vi.fn();
    const selectRows = vi.fn(async (_table: string, query: string) => query.includes('id=eq.super-1') ? [{ id: 'super-1', role: '超級管理員', status: '啟用' }] : [{ id: 'super-1' }]);
    const data = createAdminData({ insertRows: vi.fn(), selectRows, updateRows: vi.fn(), deleteRows, supabaseRequest: vi.fn() });
  ...
  it('keeps the last enabled super administrator during role or status changes', async () => {
    const updateRows = vi.fn();
    const selectRows = vi.fn(async (_table: string, query: string) => query.includes('id=eq.super-1') ? [{ id: 'super-1', account: 'super@example.com', name: 'Super', role: '超級管理員', status: '啟用' }] : [{ id: 'super-1' }]);
    const data = createAdminData({ insertRows: vi.fn(), selectRows, updateRows, deleteRows: vi.fn(), supabaseRequest: vi.fn() });
    await expect(data.updateAdminAccount('super-1', { ...input, account: 'super@example.com', name: 'Super', role: '查看人員' }, actor)).rejects.toMatchObject({ statusCode: 400 });
    expect(updateRows).not.toHaveBeenCalled();
  });
  ...
  it('enables or disables a member and audits the change', async () => {
    ...
    await expect(data.updateMemberStatus('member-1', 'disabled', actor)).resolves.toMatchObject({ id: 'member-1', status: 'disabled' });
  ...
  it('cancels automatic renewal without removing the current expiry', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('rpc/admin_update_subscription', expect.objectContaining({ body: expect.stringContaining('"p_action":"cancel"') }));
  });
  ...
  it('adjusts expiry and supports a lifetime subscription', async () => {
    const rpc = vi.fn(async () => ({ id: 'member-1' }));
    const data = createAdminData({ insertRows: vi.fn(), selectRows: vi.fn(), updateRows: vi.fn(), deleteRows: vi.fn(), supabaseRequest: rpc });
    await data.updateSubscription('member-1', { action: 'adjustExpiry', expiresAt: '2026-12-31T00:00:00Z' }, actor, new Date('2026-08-21T00:00:00Z'));
  ...
  it('confirms a transfer request through one transactional RPC', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('rpc/admin_review_transfer_request', expect.objectContaining({ body: expect.stringContaining('"p_decision":"confirmed"') }));
  });
