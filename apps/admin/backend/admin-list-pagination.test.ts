...
test('member page requests only 30 rows, preserves filtered total and scopes online summaries to returned members', async () => {
  ...
  const requestPage = vi.fn().mockResolvedValue({ items: [member], total: 61 });
  const result = await listAdminMemberPage('users', { page: '2', keyword: '', status: 'disabled' }, { request, requestPage });
  expect(result).toMatchObject({ total: 61, currentPage: 2, totalPages: 3, items: [{ lineDisplayName: '找到的會員' }] });
...
test.each(['0', '-1', '1.5', 'NaN', '100001'])('invalid page %s is rejected before reading', async page => {
