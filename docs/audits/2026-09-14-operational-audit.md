...
| 25 | 天工 API／演算法 | 使用者已確認一段式與80期早已移除；正式範圍是50期、二段式，兩者不屬待恢復項目。後續核對發現生產者仍先算80再篩50，已改為直接計算50；四彩種有限合成回歸比較結果與完整 evidence 相同。拖牌語義仍待核對；沒有任意 top-N 截斷，單一 JSONB／全量回傳的極端結果量風險尚未關閉。 |
...
- **天工規格更正**：使用者明確確認一段式與80期早已移除，不需恢復，亦不列為未完成。以最新 main `da675985b4bac2aa6847a54267c0ffc9649c4e05` 為後續修正基準，移除正式生產者固定計算80期的多餘工作，改為50期／二段式；完整 D 排除仍保留所需74期歷史。既存已完成結果沿用，不強制重算；通用 calculator／舊資料讀取的80相容能力不代表啟用產品功能。
