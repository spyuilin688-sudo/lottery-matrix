...
| 檢查點 | 現況與結論 |
| --- | --- |
| 50／80 期 | 生產計算 80 期並標示 50 期 eligibility；RPC 接受 50／80。前端目前固定送 `periodRange: 50`，沒有 80 選項。2026-09-08 `UX-CONTRACT.md` 明確要求五十期靜態顯示及請求維持 50，因此未自行重新加入 80 UI。 |
| 一段／二段 | 現行 parser、前端及現行測試只接受二段式、準2進3；一段式未支援。2026-08-21 舊規格有一段式，2026-08-31 維護規格明確移除。若要恢復一段式，需要新的產品規則決定。 |
...
| 驗證 | C/B/A 第一段均須命中，C/B 第二段命中，A 第二段是下一期預測；同完整規則若可延伸至 D 且兩段都中，排除隱藏準3進4。缺 D 歷史不可假設失敗；80 期完整 D 排除需119期歷史，50期需74期。 |
...
- 本次未重跑正式演算法、未驗證真實會員權限 HTTP 全流程、未做極端結果量壓測；Tiangong 一段式、拖牌與80期 UI 屬現行契約未支援項目，不能寫成已完成支援。
