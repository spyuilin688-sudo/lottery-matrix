...
- Initial CI exposed that the standalone admin runner did not load the root runner's dialog test shim, plus an obsolete custom-backdrop assertion. Both runners now load the same test-only dialog setup; native production behavior is unchanged. The style check covers the native backdrop and bounded scrolling. CI failure was reproduced before the test setup fix.
