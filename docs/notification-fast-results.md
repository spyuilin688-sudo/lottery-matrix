...
牌單頁的更新由牌單發布紀錄擁有：開啟時讀一次牌單，訂閱彩種對應的公開唯讀版本列；來源變更使舊牌單失效、順球或落球 manifest 有實際變更時才收到資料庫變更並重新讀牌單。同一來源版本先發布順球、後發布落球時，也以已發布球序分辨。資料庫變更串流確認可用後再讀一次，以補上初次讀取到訂閱成功之間的空窗；切回頁面、恢復網路或重新連線時再確認。連線失敗時保留原有的一小時補救讀取，離開牌單頁即移除訂閱及計時器。客戶端對版本列只能讀，不能發布假變更；其他掛載頁面的更新節奏不受此牌單專用訊號影響。
