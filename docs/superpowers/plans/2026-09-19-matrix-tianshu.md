...
- [x] 瀏覽器對照天衡與天樞，320／390px及桌面檢查設定、結果與展開驗證。
- [x] 審查整體diff與最新main差異，建立可審查分支／PR。

## Verification environment

本機 Chromium 下載受網路限制，雲端瀏覽器無法連線本機。沿用專案 GitHub Actions 的 Chromium 流程，僅執行天樞版面測試的 run 35441632617 在 320／390／1100px 三項全數通過。已人工檢視完整驗證卡及 320px 天衡對照截圖：卡片、號碼列、內距與既有高亮樣式一致；320px 天衡原有的號碼列裁切仍存在，天樞多一個高亮號碼的裝飾寬度也依循共用樣式，未改動正式 CSS。

已完成整體程式審查並建立草稿 PR #669，保留最新 main ec1c25f 的天工專屬背景。直接相關 Python、SQL、前端、TypeScript 與建置驗證通過；既有專案 CI 仍有已比對基準的失敗，包括 49 項 Vitest、12 項舊版面斷言及一項 crawler cron 字串斷言。PR 維持草稿，正式 Supabase、Railway 與 main 未變更。
