...
- Sorted cards publish immediately. A current manifest can contain only `cards.sorted`; missing current orders never resolve to previous-period cards. Actual-card readiness, independent of algorithm completion, gates the card notification.
