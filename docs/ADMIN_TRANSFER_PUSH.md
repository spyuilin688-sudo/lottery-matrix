...
- Supabase `admin-api` Edge Function：`GET/POST/DELETE /api/admin-transfer-push`，使用管理員工作階段及超級管理員檢查；裝置歸屬取自伺服器登入資料。
...
- 既有 `public/resources/admin-backend-icon.png` 與 manifest 保留，不替換已安裝的後台圖示。
