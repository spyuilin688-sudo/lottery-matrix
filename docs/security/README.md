...
Admin login checks run before password verification and wait at most 300ms; outcomes are recorded with a separate bounded call. A successful login or failed credential attempt is recorded even for superadmins, while the existing ordinary activity-log exemption is preserved. Security calls allow at most eight outstanding underlying operations per instance, even if a dependency ignores abort. Warnings retain the `security-observation-unavailable` event name and add JSON metadata: a fixed reason (`capacity`, `deadline`, `http`, `degraded`, or `exception`), stage (`admission`, `config`, `identity`, `request`, or `response`), category, outcome, elapsed milliseconds, and HTTP status only for an HTTP rejection. They remain limited to one warning per minute per instance; a late failure of an already timed-out operation does not produce a second incident. Credentials, addresses, headers, upstream bodies and exception messages are never logged. The trusted source is the Edge runtime's verified `clientIp`, with the existing trusted Lambda `requestContext.http.sourceIp` fallback, never unverified `X-Forwarded-For`. If absent or invalid, monitoring is aggregate-only. Auth is never weakened by observation failure.
