...
def persist_failure(exc: BaseException) -> None:
    ...
    diagnostic = [
        f'exception={type(exc).__name__}: {exc}', '', traceback.format_exc(), '',
        '--- command log ---', *COMMAND_LOG[-20:],
    ]
...
def merge_remote(branch: str, label: str) -> None:
    ...
    if result.returncode != 0:
        ...
        if unexpected or not conflicts:
            raise RuntimeError(f'{label}: unexpected merge conflicts {sorted(conflicts)}')
...
def restore_explore_formula_core() -> None:
    ...
    if len(validation_pattern.findall(section)) != 1:
        raise RuntimeError('validationFormula source not unique')
...
def write_canonical_app_formula_test() -> None:
    ...
    if source.count(marker) != 1:
        raise RuntimeError('app shell insertion marker not unique')
...
def update_validation_contract_test() -> None:
    ...
    if start < 0 or end < 0:
        raise RuntimeError('validation contract anchors not found')
...
def update_matrix_explore_page_tests() -> None:
    path = Path('src/__tests__/MatrixExplorePage.test.tsx')
    source = path.read_text()

    # Backend Explore rules are integer-valued. Make the shared fixture a real
    # add-rule relation: base 14 + rule 8 = hit 22.
    source = replace_once(
        source,
        "rules: [{ value: 14.24, display: '+14.24', algorithmType: '加減' }],",
        "rules: [{ value: 8, display: '+8', algorithmType: '加減' }],",
        'default rule fixture',
    )
    source = replace_once(source, 'candidateRules: [14.24],', 'candidateRules: [8],', 'default candidate rule')
    source = replace_once(
        source,
    ...
    if source.count(".toContain('+14.24')") != 2:
        raise RuntimeError(f'legacy +14.24 contain assertions count={source.count(".toContain(\'+14.24\')") }')
    source = source.replace(".toContain('+14.24')", ".toContain('+8')")

    old_drag_history = '''        historicalValidation: [{
    ...
    new_drag_history = '''        historicalValidation: [{
          ...exploreValidationEnvelope.validation.ruleSets[0].historicalValidation[0],
    ...
    source = replace_once(
        ...
        'drag reverse truncation',
    )
    old_step = '''  expect(firstGroup!.querySelector('.explore-validation-number--source')).toBeNull();
...
test.each(['六合彩', '大樂透'] as const)'''
    new_step = '''  expect(firstGroup!.querySelector('.explore-validation-number--source')).toBeNull();
  expect([...firstGroup!.querySelectorAll('.explore-validation-number--step')].map((node) => node.textContent)).toEqual(['09', '28', '38']);
  expect(validation.querySelectorAll('.explore-validation-group')[1].querySelector('.explore-validation-number--hit')?.textContent).toBe('44');
});

test.each(['六合彩', '大樂透'] as const)'''
    source = replace_once(source, old_step, new_step, 'drag hit-number assertion')
    ...
    source = replace_once(
        ...
        'legacy numeric typed resolution',
    )

    old_sum_rule = "        rules: [{ value: 14.24, display: '合值14.24', algorithmType: '合值' }],"
    new_sum_rule = '''        rules: [{ value: 36, display: '合值36', algorithmType: '合值' }],
    ...
    source = replace_once(source, old_sum_rule, new_sum_rule, 'sum-rule fixture')
    source = replace_once(
        source,
    ...
    source = replace_once(
        ...
        'sum computed formula',
    )

    if '14.24' in source:
        raise RuntimeError('invalid decimal Explore rule fixture remains in MatrixExplorePage.test.tsx')
    path.write_text(source)
...
def verify_contracts() -> None:
    ...
    matrix_test = Path('src/__tests__/MatrixExplorePage.test.tsx').read_text()
    required_feature = [
        'const [referenceLoadState, setReferenceLoadState]',
        'const [historyError, setHistoryError]',
        'fetchMemberReferralSummary()',
        'price: "$2,880"', 'price: "$5,580"', 'price: "$17,800"',
    ...
    explore = feature[feature.find('function ExploreValidationProcess('):feature.find('\nfunction TianyanValidationProcess(')]
    for needle in [
        ...
        if needle in explore:
            raise RuntimeError(f'stale Explore contract remains: {needle}')
    if 'export function MatrixCorePage(' in feature:
        raise RuntimeError('PR #277 unreachable MatrixCorePage was reintroduced')
    for needle in [
        'padding-block: 0.3px;', 'padding-inline: 0.7px;', 'border-width: 0.7px;',
        'flex-basis: auto;', 'padding-block: 12px;',
        '.matrix-explore-main-screen:not(.matrix-tianyan-screen) .explore-validation-formula-position {',
        'gap: 1px;', 'font-size: calc(.8em - 2px);', 'margin-inline: 2px;',
    ]:
    ...
    if '14.24' in matrix_test:
        raise RuntimeError('invalid decimal Explore test rule remains')
    for filename in [
    ...
    if Path('.github/workflows/restore-explore-formula-core-once.yml').exists():
        raise RuntimeError('temporary PR #284 restore workflow remains')
...
def main() -> None:
    ...
    for temporary in [
        '.github/workflows/restore-explore-formula-core-once.yml',
        'integration-diagnostic.txt',
    ]:
    ...
    update_matrix_explore_page_tests()
    write_post_guard()
    verify_contracts()
    run(['git', 'diff', '--check'])
    run([
