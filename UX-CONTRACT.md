...
The notification source supplies a fresh date and sorted numbers. Persist that information and enqueue the deduplicated lottery-result notification immediately; estimate its period as the preceding period plus one. Home, TongXing, NumberReference and history show available current information. Missing actual-order balls and their special ball remain absent, never reconstructed from sorted numbers or an older draw. Mounted readers share the accepted one-hour fallback cadence and revalidate on data revision, visibility or network recovery (see `docs/qa/2026-09-14-resumed-verification.md`). Formal enrichment and period correction invalidate derived caches.
...
- Check an authenticated member session (LINE or Google) at the entry click, before changing the screen or shortcut state. Guests see the existing login dialog and remain on their originating page when it closes; never redirect them home. This applies to normal navigation, opening a saved shortcut, selecting a shortcut, and the first tap on the custom-condition icon. Logged-in custom-condition entry retains its existing double-tap behavior. The shared feature router additionally prevents protected content from mounting without a member session. Session-read failures have a separate retry-later prompt. Sign-out removes page access, and stale session reads must not restore it. Existing server and entitlement checks remain authoritative.
...
- Subscription management records an already completed full refund, chargeback or transaction cancellation against a confirmed payment. This action records the result; it does not transfer money, change subscription dates, cancel auto-renew, or reopen the original confirmed transfer request.
...
- The existing administrator session, subscription-module edit permission and operation edit permission are all required. The backend supplies the authenticated actor; client-supplied actor fields never authorize or identify the operation. Reading payment records requires subscription-module view permission.
- Confirmation states the member, payment, amount, result, reason and referral consequence. Submission is locked through confirmation, mutation and refresh. Cancellation sends no write; leaving the panel or losing edit permission invalidates a pending confirmation. Failure preserves the reason and allows an explicit retry. A completed mutation followed by failed refresh remains a completed mutation with a refresh warning.
...
- The service-only database operation locks one payment and commits its terminal status, immutable actor/time/reason snapshot and applicable audit together. A same-status retry returns original metadata without another audit; conflicting terminal decisions and non-confirmed payments are rejected. Existing super-administrator audit exemption and administrator deletion behavior remain unchanged, while payment-row actor metadata preserves the historical trace.
...
- Lottery tabs sit 14px below the frame and retain the existing lottery-change behavior. Core pages reuse the exact draw-sheet LotteryTabs component and styling (36px, 14px type, 2px text-width gold active underline), with keyboard selection and without the redundant lottery select.
...
- One 14px header bottom margin separates the frame and the next visible content (commits `7905e45` / `2eb6348`). History, number reference and tongxing settings share the title border with one internal divider; their floating state is anchored to the same sticky header.
...
- Bright/standard/muted tiers are 1px; internal separators use the shared 38% gold token (commit `8ec6ebbe`). Selected controls use standard gold text/border and the shared 6% surface.
