...
- 依使用者要求，暫時隱藏「我的」及「管理訂閱」內的「訂閱方案／收費標準」入口。
- `src/subscription-purchase-visibility.ts` 統一控制購買介面顯示；關閉時，既有 `pro-plans` 與 `manual-transfer` 頁面由共用 router 顯示會員頁，購買頁不掛載。
- 保留目前方案、到期日、付款紀錄、登入與原有手機版面；此設定不修改會員方案、演算法使用權限或後端付款 API。
- 使用者尚未選定免費開放的控制方式，本次不啟用免費演算法權限。
