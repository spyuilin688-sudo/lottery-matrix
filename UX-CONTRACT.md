...
- Notebook list pagination shows 10 notes per page with numbered buttons, `aria-current="page"`, and the card page’s shared transparent/gold-underlined switch styling. Add/delete stay left and pagination right on the same row; no note count is displayed. A shared static notebook watermark spans the list and toolbar without intercepting input. Successful creation returns to page 1; editing preserves the page; deletion clamps to the last remaining page.
