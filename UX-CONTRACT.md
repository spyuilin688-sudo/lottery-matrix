...
- Header actions remain inside the fixed frame and scroll with the sticky header, so filters can be opened without scrolling back to the top. History, number reference and tongxing share a single bordered header/settings card. The 24px borderless settings control uses muted gold 12px text, a funnel, a state chevron, aria-expanded and aria-controls. Its action column is 86px; other header actions retain their default geometry. Initial settings are in normal flow; reopening overlays results from the same sticky header with a 68px placeholder. Settings remain mounted so draft values persist; Escape closes and restores trigger focus. No viewport measurement or portal is used for these settings.
