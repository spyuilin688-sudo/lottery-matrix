...
export function MatrixGuidePage({ onNavigate }: { onNavigate: Navigate }) {
  ...
  const allSections: GuideSection[] = [
    ...
    },
    {
      ...
      blocks: [
        { title: "查詢方式", items: ["選擇彩種與號碼順序；天天樂固定使用順球。", "可依年、月、日設定日期條件，或選擇1000期、3000期、5000期、所有期數的探索範圍。", "按下「開始探索」後顯示符合條件的歷史開獎紀錄。"] },
    ...
    },
    {
      ...
      blocks: [
        { title: "探索設定", items: ["彩種：今彩539、天天樂、六合彩、大樂透。", "探索期數：二期、七期、十三期。", "版路類型：加減版路、合值版路、拖牌版路。", "命中條件：準4+ (鎖定 1 碼)、準5+ (鎖定 2 碼)。", exploreAccessCopy] },
    ...
    },
    {
      ...
      blocks: [
        { title: "天衡設定", items: ["彩種：今彩539、天天樂、六合彩、大樂透。", "天衡期數：三期、十三期。", "版路類型：加減版路、合值版路、拖牌版路。", "天衡條件：準5+ (鎖定 1 碼)、準6+ (鎖定 2 碼)。"] },
        ...
        { title: "進階天衡設定", items: ["號碼順序：今彩539、六合彩、大樂透可選依號碼由小到大排序或依實際開獎順序排序；天天樂固定依號碼由小到大排序。", "天衡日期：可選本日 (最新)、昨日 (上1期)、前日 (上2期)。", "標準範圍包含上 1 ~ 7 期、當期及當期之後至結果期前一期的參考球位；完整範圍向上擴大至 14 期，皆不包含結果期。", tianhengAccessCopy] },
    ...
    },
    {
      ...
      blocks: [
        { title: "天樞設定", items: ["彩種：今彩539、天天樂、六合彩、大樂透。", "天樞期數：三期、十三期。", "版路類型：加減版路、合值版路、拖牌版路。", "天樞條件：準5+ (鎖定 1 碼)、準6+ (鎖定 2 碼)。"] },
    ...
    },
    {
      ...
      blocks: [
        ...
        { title: "查看更多開獎紀錄", items: ["首頁點選查看更多紀錄，可查閱歷史開獎紀錄。", "號碼對照單可選擇1000期、3000期或5000期。"] },
