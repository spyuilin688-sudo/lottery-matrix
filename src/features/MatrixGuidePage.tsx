  ...
  const allSections: GuideSection[] = [
    ...
    },
    {
      ...
      blocks: [
        ...
        { title: "查看結果", items: ["按下「開始探索」後，查看重複號碼統計與探索結果。", subscriptionPurchaseVisible ? "結果顯示位置、號碼、預測期、連準次數、預測及版路類型。" : "結果顯示位置、號碼、查詢期、連準次數、結果及版路類型。", "可使用同碼與連準篩選，並展開每條版路查看驗證過程。"] },
      ],
    },
    {
      title: "Matrix 天衍",
    ...
    },
    {
      ...
      blocks: [
        ...
        { title: "定位版路", items: ["定位版路不使用鎖定條件；探索、第一段與第二段各使用1個球位路徑。", subscriptionPurchaseVisible ? "第一段驗證3個球位；第二段驗證前2個球位，第3個球位產生預測。" : "第一段驗證3個球位；第二段驗證前2個球位，第3個球位產生結果。"] },
        { title: "球位與版路", items: ["探索球位、第一段球位與第二段球位可選固定、依序遞增或依序遞減。", "第一段與第二段的版路類型皆可選加減版路或合值版路。"] },
        { title: "查看結果", items: [subscriptionPurchaseVisible ? "按下「開始探索」後顯示間距期數、預測位置、預測及版路類型。" : "按下「開始探索」後顯示間距期數、查詢位置、結果及版路類型。", "可展開版路查看驗證過程。"] },
      ],
    },
    {
      title: "Matrix 狀態",
      ...
      blocks: [
        ...
        { title: "查看方式", items: ["切換彩種查看各自狀態。", "點擊狀態下拉可展開符合觸發條件的版路。", subscriptionPurchaseVisible ? "每條版路顯示位置、號碼、預測期、連準次數及版路類型。" : "每條版路顯示位置、號碼、查詢期、連準次數、結果及版路類型。", "符合一組以上觸發條件時，各組內容以間隔區分。"] },
