  ...
  const allSections: GuideSection[] = [
    ...
    },
    {
      ...
      blocks: [
        ...
        { title: "鎖定條件與驗證", items: ["系統依所選彩種、探索期數、號碼順序與球位建立鎖定條件，再以歷史資料完成版路驗證。", "今彩539與天天樂每期使用5個球位；六合彩與大樂透使用6個正碼球位及特別號，因此不同彩種的可比對組合數不同。", "加減、合值與拖牌依各自規則進行驗證，實際結果以目前演算法版本運算為準。"] },
        { title: "進階探索設定", items: ["號碼順序：今彩539、六合彩、大樂透可選依號碼由小到大排序或依實際開獎順序排序；天天樂固定依號碼由小到大排序。", "探索日期：可選本日 (最新)、昨日 (上1期)、前日 (上2期)。", "標準範圍：上1～7、當期、下 N 至結果期前一期；不包含結果期。", "完整範圍：上1～14、當期、下 N 至結果期前一期；不包含結果期。"] },
    ...
    },
    {
      ...
      blocks: [
        ...
        { title: "比對方式", items: ["每組以同一期的兩個不同球位與對應號碼作為條件，查找歷史中相同球位及號碼同時出現的紀錄。", "加減版路與合值版路依所選範圍比對參考球位；拖牌版路使用第一個條件球位的號碼進行驗證。", "來源條件固定包含兩個球位；「鎖定1碼／2碼」是結果規則的設定，不會改變來源條件的球位數。"] },
        { title: "進階天衡設定", items: ["號碼順序：今彩539、六合彩、大樂透可選依號碼由小到大排序或依實際開獎順序排序；天天樂固定依號碼由小到大排序。", "天衡日期：可選本日 (最新)、昨日 (上1期)、前日 (上2期)。", "標準範圍包含上1～7期、當期及當期之後至結果期前一期的參考球位；完整範圍向上擴大至14期，皆不包含結果期。", "十三期與完整範圍依目前帳號權限開放。"] },
    ...
    {
      title: "Matrix 天衍",
      summary: "固定使用十三期與完整範圍，以複合版路進行探索，命中條件固定為準5+ (鎖定2碼)。",
      blocks: [
        { title: "天衍設定", items: ["彩種：今彩539、天天樂、六合彩、大樂透。", "天衍期數固定為十三期，天衍範圍固定為完整範圍。", "版路類型固定使用複合版路。", "命中條件固定為準5+ (鎖定2碼)。", "進階設定可調整號碼順序與天衍日期；天天樂固定依號碼由小到大排序。"] },
    ...
    {
      title: "Matrix 天工",
      summary: "固定使用五十期、二段式與準2進3，依三組等距來源、球位走向與兩段版路類型定位結果。",
      blocks: [
        { title: "天工設定", items: ["彩種：今彩539、天天樂、六合彩、大樂透。", "天工期數固定為五十期。", "流程固定為二段式，命中條件固定為準2進3。", "天工正式運算使用依號碼由小到大排序的資料。"] },
        ...
        { title: "準3進4排除", items: ["系統會沿相同間距向更舊的 D 組回推。", "若 D 組以相同完整規則在第一段與第二段都成立，代表版路可延伸為準3進4，該候選直接排除。", "若歷史資料不足以完成 D 組排除檢查，正式結果不輸出該候選。"] },
    ...
    },
    {
      ...
      blocks: [
        ...
        { title: "自訂觸發條件", items: ["各彩種的自訂觸發條件分開設定。", "在 Matrix 狀態頁面，連續點擊右上角設定按鈕兩下，即可開啟「自訂觸發條件」。", "自訂觸發條件依目前帳號權限開放，可重置或儲存目前設定。"] },
        { title: "四狀態條件設定", items: ["啟動、聚合、共振、臨界四個觸發狀態分別設定；沉寂不另設觸發條件。", "每組條件包含命中條件、連準次數、版路類型、號碼順序與同碼數量。", "同一組內有多列條件時，需全部符合。"] },
      ],
    },
    {
      title: "Matrix 同星",
      summary: "輸入2至3個指定號碼後，查詢這些號碼出現的條件期，以及指定間隔後的開獎結果。",
      blocks: [
        { title: "設定條件", items: ["選擇彩種及號碼順序；天天樂固定使用順球。", "至少輸入2個、最多3個號碼，號碼不可重複。", "「之後下」可選擇1至30期，再按「開始探索」。"] },
        { title: "結果內容", items: ["每組結果成對顯示條件期與指定間隔後的結果期，左側顯示期數與日期，右側顯示開獎號碼。", "今彩539與天天樂顯示5個號碼；六合彩與大樂透顯示6個號碼及特別號。"] },
      ],
    },
    {
      title: "號碼對照單",
      ...
      blocks: [
        { title: "查詢設定", items: ["選擇彩種、歷史範圍 (1000/3000/5000期) 及號碼順序；天天樂固定使用順球。", "可輸入0至3個探索號碼；空白格不參與探索，號碼不可重複。"] },
    ...
    },
    {
      ...
      blocks: [
        { title: "快捷", items: ["點擊底部「快捷」開啟目前設定的功能。", "在首頁設定按鈕連續點擊兩下可設定快捷功能。"] },
    ...
    },
    {
      ...
      blocks: [
        ...
        { title: "查看更多開獎紀錄", items: ["首頁近10期開獎號碼點選查看更多紀錄，可查閱歷史開獎號碼。", "號碼對照單可選擇1000期、3000期或5000期。"] },
