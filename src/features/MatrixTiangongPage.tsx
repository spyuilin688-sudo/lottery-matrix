...
export function TiangongValidationProcess({ validation, loading, lottery = "今彩539", predictionNumber, predictedPosition, item }: { validation?: TiangongValidation; loading: boolean; lottery?: LotteryId; predictionNumber?: string; predictedPosition?: number; item?: TiangongApiRow }) {
  ...
  const oldestFirstGroups = ["C", "B", "A"] as const;
  const groups = [...rows].sort((a, b) => oldestFirstGroups.indexOf(a.group as typeof oldestFirstGroups[number]) - oldestFirstGroups.indexOf(b.group as typeof oldestFirstGroups[number]));
      ...
      <ExploreValidationSummary layout="tianyan">
        <span className="tianyan-validation-summary-lines" aria-label="版路摘要">
          <span className="tianyan-validation-summary-row">
            <span className="validation-summary-primary">{summaryDirection(item.exploreDirection)}</span>
          ...
          <span className="tianyan-validation-summary-row">
            <span className="validation-summary-primary">{summaryDirection(item.firstStageDirection)}</span>
            ...
            {separator}<span className="validation-summary-primary">{summaryDirection(item.secondStageDirection)}</span>
