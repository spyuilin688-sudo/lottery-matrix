...
export function BrandHeader({ title, onBack, backHref, action, actionPlacement = "bottom-right", settings, showBack = true }: {
  ...
  const headerStyle = title === TIANGONG_HEADER_TITLE ? "tiangong" : CORE_HEADER_TITLES.has(title) ? "flow" : GEOMETRIC_HEADER_TITLES.has(title) ? "geometric" : "gold-arc";
