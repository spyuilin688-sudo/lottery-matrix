    ...
    <FeatureShell title="Matrix 天工" onNavigate={onNavigate} backTarget="explore" className="matrix-explore-screen matrix-explore-main-screen matrix-explore-layout matrix-tiangong-screen">
      <section className="panel explore-settings tiangong-settings">
        ...
        <div className="setting-grid">
          ...
          <div className="tiangong-setting-row" role="group" aria-label="探索球位"><span>探索球位</span><div className="segmented three">{positionOptions.map((value) => <button type="button" data-selected={searchPositions.includes(value)} onClick={() => toggle(value, searchPositions, setSearchPositions)} key={value}>{value}</button>)}</div></div>
          <div className="tiangong-setting-row" role="group" aria-label="第一段球位"><span>第一段球位</span><div className="segmented three">{positionOptions.map((value) => <button type="button" data-selected={firstPositions.includes(value)} onClick={() => toggle(value, firstPositions, setFirstPositions)} key={value}>{value}</button>)}</div></div>
          <div className="tiangong-setting-row" role="group" aria-label="第一段版路類型"><span>第一段版路類型</span><div className="segmented two">{roadOptions.map((value) => <button type="button" data-selected={firstRoads.includes(value)} onClick={() => toggle(value, firstRoads, setFirstRoads)} key={value}>{value}</button>)}</div></div>
