...
export function NumberReferencePage({ onNavigate }: { onNavigate: Navigate }) {
  ...
  return (
    <FeatureShell title="號碼對照單" onNavigate={onNavigate} className="number-reference-screen">
        ...
        <div>
          {inputs.map((v, i) => (
            <input
              ...
              onChange={(event) => setInputs(inputs.map((x, index) => (
                index === i ? event.target.value : x
              )))}
            />
          ))}
          <button type="button" className="gold-button branded-explore-action"><MagnifyingGlassIcon />開始探索</button>
        </div>
      </section>
                ...
                <span>
                  {displayedNumbers.map((num, index) => {
                    ...
                    return (
                      <button
                        type="button"
                        data-auto-match={inputIndex >= 0 ? inputIndex + 1 : 0}
...
export function CalculatorPage({ onNavigate }: { onNavigate: Navigate }) {
  ...
  return (
    <FeatureShell title="連碰立柱計算機" onNavigate={onNavigate} className="calculator-screen">
...
export function NotificationsPage({ onNavigate }: { onNavigate: Navigate }) {
  ...
  const rows = [
    ...
    ["card", "Matrix 牌單", "今彩539、天天樂、六合彩、大樂透", "/resources/notify-card-v2.png?v=20260809-4"],
    ["collision", "Matrix 摘星", "", "/resources/notify-collision-v2.png?v=20260809-4"],
    ["expiry", "Matrix Pro", "提前1日、提前3日、提前7日", "/resources/notify-expiry.png"],
    ["system", "系統通知", "維護、更新", "/resources/notify-system-v2.png?v=20260809-4"],
  ] as const;
  ...
  const renderSettings = (key: string, title: string, subtitle: string) => {
    ...
    if (key === "status") return <div className="notification-lottery-settings">{lotteries.map((lottery) => <fieldset className="notification-lottery-row" key={lottery}><legend>{lottery}</legend><div>{matrixStatuses.map((option) => <label className="notification-choice" key={option}><input type="checkbox" checked={statusOptions[lottery]?.includes(option)} onChange={() => toggleNestedOption(setStatusOptions, lottery, option)} /><span>{option}</span></label>)}</div></fieldset>)}</div>;
