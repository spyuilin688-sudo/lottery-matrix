...
it('routes Tianheng into the canonical Explore layout with its shared text header and 3/13 controls', async () => {
  render(<FeaturePageRouter screen="tianheng" onNavigate={vi.fn()} />);
  await act(async () => {});
  const header = screen.getByRole('heading', { level: 1, name: 'MATRIX 天衡' }).closest('header');
...
it.each([
  ...
  fireEvent.click(screen.getByRole('button', { name: '進階天衡設定' }));
...
it('orders the drag summary in exactly two rows', async () => {
  ...
  expect(rows[0]).toHaveTextContent('開05 第 1 顆、同期 18 第 4 顆');
...
it('places both source numbers on the first summary row and the complete rule on the second', async () => {
  ...
  const rows = await screen.findAllByTestId('tianheng-summary-row');
  expect(rows).toHaveLength(2);
  expect(rows[0]).toHaveTextContent('開02 第 1 顆、同期 09 第 3 顆');
