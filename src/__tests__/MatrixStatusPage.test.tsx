...
const statusApi = vi.hoisted(() => ({
  fetchMatrixStatus: vi.fn(), fetchMatrixStatusValidation: vi.fn(),
}));
...
test('狀態尚未回傳時顯示載入中，不把未知數量當成零或空結果', async () => {
  ...
  await act(async () => finish({
    ...
    counts: { ACTIVE: 0, FOCUS: 0, RESONANCE: 0, CRITICAL: 0 }, cards: [], detailLocked: true,
  }));
...
test('standard status keeps results and lottery switching without retired custom settings', async () => {
  const navigate = vi.fn();
  const { container } = render(<MatrixStatusPage onNavigate={navigate} />);
  await screen.findByText('2 組');
  expect(screen.queryByRole('button', { name: /自訂觸發條件/ })).toBeNull();
  expect(container.querySelector('.product-header__actions')).toBeNull();
  fireEvent.click(screen.getByRole('radio', { name: '天天樂' }));
  await waitFor(() => expect(statusApi.fetchMatrixStatus).toHaveBeenCalledWith('天天樂'));
  expect(navigate).not.toHaveBeenCalled();
});
