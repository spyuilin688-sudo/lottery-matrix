...
// JSDOM does not resolve custom properties in border shorthands. Keep their
// token contract explicit; pwa-frame-system.spec.ts checks rendered browser styles.
function expectOwnedDeclaration(style: HTMLStyleElement, element: Element, selector: string, property: string, value: string) {
  const rule = [...style.sheet!.cssRules].find((entry): entry is CSSStyleRule =>
...
describe("production member shell", () => {
  ...
  it("keeps isolated preview headings and result rows readable", () => {
    ...
    expectOwnedDeclaration(previewStyle, positionTag!, ".explore-result-preview-screen .road-results .tag", "border", "1px solid var(--pwa-frame-tertiary)");
  ...
  it("matches the reference navy surfaces and restrained gold and blue borders", () => {
    ...
    expectOwnedDeclaration(productionStyle, panel!, ".panel", "border", "1px solid var(--pwa-frame-secondary)");
    expect(getComputedStyle(validation!).borderTopWidth).toBe("1px");
    expect(getComputedStyle(validation!).borderTopColor).toBe("var(--pwa-frame-secondary)");
    expect(getComputedStyle(validation!).borderBottomColor).toBe("var(--pwa-frame-secondary)");
    ...
    expectOwnedDeclaration(productionStyle, summaryCard!, ".explore-validation-summary-card", "border", "1px solid var(--pwa-frame-secondary)");
    expect(getComputedStyle(summaryTag!).backgroundColor).toBe("rgba(10, 14, 24, 0.98)");
    expectOwnedDeclaration(productionStyle, summaryTag!, ".explore-validation-summary-card > .explore-validation-consecutive-tag", "border", "1px solid var(--pwa-frame-secondary)");
    ...
    expectOwnedDeclaration(productionStyle, prediction!, ".explore-validation-prediction", "border", "1px solid var(--pwa-frame-secondary)");
    expectOwnedDeclaration(productionStyle, issues!, ".explore-validation-issues,\n.explore-validation-numbers-card,\n.explore-validation-formulas", "border", "1px solid var(--pwa-frame-divider)");
    expectOwnedDeclaration(productionStyle, secondFormula!, ".explore-validation-number-row:nth-child(n + 2),\n.explore-validation-issue:nth-child(n + 2),\n.explore-validation-formula-row:nth-child(n + 2)", "border-top", "1px solid var(--pwa-frame-divider)");
  ...
  it("styles only the expanded summary consecutive value as a tag card", () => {
    ...
    expectOwnedDeclaration(previewStyle, summaryTag!, ".explore-validation-summary-card > .explore-validation-consecutive-tag", "border", "1px solid var(--pwa-frame-secondary)");
  ...
  it("keeps the approved unexpanded result hierarchy after shared styles load", () => {
    ...
    expectOwnedDeclaration(productionStyle, filterButton, ".explore-consecutive-filter-button", "border", "1px solid var(--pwa-frame-tertiary)");
    expectOwnedDeclaration(productionStyle, filterButton, ".explore-consecutive-filter-button", "color", "var(--lottery-text-secondary)");
