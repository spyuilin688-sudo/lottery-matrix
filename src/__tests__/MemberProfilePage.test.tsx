  ...
  supabase.auth.getSession.mockReset().mockResolvedValue({
    data: { session: {
      ...
      user: {
        ...
        user_metadata: { name: "LINE 會員暱稱" },
        identities: [{ provider: "custom:line", provider_id: "line-real" }],
      },
    } },
    error: null,
  });
...
describe("ProfilePage member API", () => {
  it("購買關閉時保留目前訂閱狀態，隱藏購買入口、付款紀錄與退款規範", async () => {
    ...
    expect(document.querySelector(".subscription-status-card")).not.toBeNull();
    ...
    // Current subscription status keeps its artwork even when purchasing is hidden.
    expect(artworkTop + artworkHeight).toBeGreaterThanOrEqual(740);
    expect(artwork.querySelector(".subscription-information-art")).not.toBeNull();
  ...
  it("會員暱稱為資訊文字，不呈現輸入框邊線或供應商 ID", async () => {
    render(<ProfilePage onNavigate={vi.fn()} />);
    await screen.findByRole("button", { name: "登出" });
    const nickname = await screen.findByText("LINE 會員暱稱");
    expect(getComputedStyle(nickname).borderTopWidth).toBe("0px");
    expect(screen.queryByText("LINE ID：line-real")).not.toBeInTheDocument();
  ...
  it("顯示 LINE 暱稱，長暱稱在固定框內縮小並省略", async () => {
    ...
    expect(screen.queryByText(`LINE ID：${lineId}`)).not.toBeInTheDocument();
  ...
  it("以登入 session 顯示暱稱，並以會員 API 顯示方案與到期日", async () => {
    purchaseSetting.visible = true;
    vi.useFakeTimers();
    ...
    expect(screen.getByText("LINE 會員暱稱")).toBeInTheDocument();
...
it("開啟購買開關後恢復入口，關閉後隱藏入口但保留目前訂閱狀態", async () => {
  const onNavigate = vi.fn();
