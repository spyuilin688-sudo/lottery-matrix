...
describe("homepage lottery switcher structure", () => {
  it("uses the shared Matrixbba artwork as the only homepage switcher visual source", () => {
    ...
    expect(source).toContain('<LotterySwitcher selected={selected} onChange={setSelected} className="home-switcher-box" />');
    expect(source).not.toContain('<LotterySwitcher selected={selected} onChange={setSelected} className="home-switcher-box" independentCards />');
  });
});
