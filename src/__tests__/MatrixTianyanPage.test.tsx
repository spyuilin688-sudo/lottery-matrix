...
test('天衍版路摘要固定最多兩列，即使驗證資料包含更多規則', async () => {
...
test('天衍版路展開後每組驗證過程最多四列', async () => {
  render(<MatrixExplorePage onNavigate={vi.fn()} title="Matrix 天衍" roadTypes={['複合版路']} />);
  fireEvent.click(screen.getByRole('button', { name: '開始探索' }));
  fireEvent.click(await screen.findByRole('button', { name: /展開版路/ }));

  const region = await screen.findByRole('region', { name: '天衍驗證過程' });
  const groups = [...region.querySelectorAll('.explore-validation-group')];
  expect(groups.length).toBeGreaterThan(0);
  for (const group of groups) {
    expect(Number(group.getAttribute('data-row-count'))).toBeLessThanOrEqual(4);
  }
});
