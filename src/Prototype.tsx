...
export function LatestDrawCard({
  ...
  const displayedNumbers =
    order === "順球"
      ? [...result.numbers].sort((a, b) => Number(a) - Number(b))
      : result.numbers;
  ...
  return (
    <section
      className={`latest-draw-card ${className}`.trim()}
      data-lottery={lottery}
      aria-label={`${lottery}最新開獎資訊`}
      data-testid="latest-draw-card"
    >
      <div className="draw-toolbar">
        <div className="draw-meta" data-empty={!hasMeta}>
          {result.issue ? (
            <div className="draw-issue">
              <span>第</span>
