...
let passwordRecoveryClient: SupabaseClient | null = null;
...
/** Only the email callback can supply this session; never reuse a signed-in member. */
export function getPasswordRecoveryClient() {
  passwordRecoveryClient ??= createClient(url, anonKey, {
    auth: {
      ...
      autoRefreshToken: false,
      detectSessionInUrl: true,
      storageKey: 'matrix-password-recovery',
    },
    global: { fetch: fetchWithPolicy },
  });
