...
「我的」頁的目前訂閱狀態卡維持已確認 A+B 黑金框架，方案資訊與到期資訊改採單欄上下排列。左側皇冠保留為方案識別，原中央垂直裝飾分隔線移除。免費會員、月費方案、季費方案、年費方案、終生／終身方案共用相同資訊結構、CTA 與互動流程，只依方案切換資訊區背景、五款獨立皇冠識別與方案名稱金色色階；舊皇冠由原素材遮罩移除，不與方案皇冠重疊。方案、說明、到期日及剩餘天數仍使用既有資料來源與邏輯；付費方案說明維持單行「Matrix Pro 權限」，免費會員仍顯示「核心功能體驗」。卡片高度依內容自然延伸，既有 SVG 上下分片維持比例，中段直線框隨內容延伸；最後一行與底部訂閱入口保留至少 8px，入口維持至少 44px 觸控高度。320–430px 保持不超框、不重疊及無水平溢位。
