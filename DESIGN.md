...
聯絡客服、問題回報、商務合作維持三張卡片，標題保留金色與 15px；信箱與電話使用 14px 淺灰白 `#c9c2b8` 及底線，hover 使用 `#f1ece3`，與標題金色區分。三張卡片的信箱文字前統一顯示「信箱：」。電話列與信箱間距 8px。電話沿用 `ContactSupportPhonePortal`，不重複新增；`src/feature-pages.css` 擁有此頁聯絡資訊樣式。
