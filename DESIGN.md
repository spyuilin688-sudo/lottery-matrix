...
Matrix 探索與天衍的驗證過程，依鎖定條件整組交替使用 `#12243A`、`#0E1D30`。左、中、右三欄共用同組底色，欄間與組間間距皆透出頁面黑色背景。既有結果的期號、特別號與欄寬依產生該結果的彩種呈現，探索設定尚未提交時不改變結果版面。版路摘要的公式序列以獨立文字節點呈現，例如 `+5`、`.`、`15`，合值序列前加 `合值`；相鄰文字節點間距皆為 1px。
