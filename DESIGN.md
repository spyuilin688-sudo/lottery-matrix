...
主標題暖金色 `#f0c85f`、700、20px 上限，副標 9px 灰金色，靠左對齊、兩行間距 4px。主副標各自固定單行，不以換行、刪字或省略號改變頁名。主標依文字長度使用共用 regular／medium／long 字級範圍，搭配標題內容容器的 cqi 自動縮小；不得以額外 inline style、補償位移或覆寫控制字級。篩選、重設與探索設定等原有標題操作保留在卡內右下方的 grid 區域，標題卡維持 68px；工具頁保留 sticky 行為與浮動設定面板的定位讀取。所有頁面共用固定 35px 文字列、3px 列距與 18px 操作列，不因按鈕是否存在改變排列。主標行盒固定 22px，使用 18px 零寬基線支架對齊縮字後的文字；副標起點、Logo 位置與文字左緣固定。
