...
Matrix 探索與天衍的驗證過程，依鎖定條件整組交替使用 `#152A42`、`#0E1D30`。左、中、右三欄共用同組底色，欄間與組間間距皆透出頁面黑色背景。既有結果的期號、特別號與欄寬依產生該結果的彩種呈現，探索設定尚未提交時不改變結果版面。版路摘要的公式序列以獨立文字節點呈現，加減版路例如 `+5`、`.`、`15`，合值序列為 `合值`、`5`、`.`、`15`（不顯示 `+`）；相鄰文字節點間距皆為 1px。
...
沿用 16px 頁面留白、12px 容器內距，分隔線上下各 12px；頁名 18px、章節標題 15px、正文 13px／1.65 行高、段距 8px。Matrix Core 的探索／天衍／天工以無圓點子清單縮排，功能項目間距為 4px；彩種採兩欄兩列，訂閱方案每個方案以完整一行靠左呈現，方案之間沿用 6px 行距。郵件連結使用與標題相同的暖金色並保留底線。原文、價格、天數、章節及閱讀順序不變，窄螢幕允許換行，容器隨內容增加高度。
