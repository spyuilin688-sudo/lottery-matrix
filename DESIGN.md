...
筆記本只提供列表與筆記編輯，沿用頁面左右 16px、標題卡下方 8px。`src/feature-pages.css` 是筆記本版面的樣式來源；依使用者確認的 B 方案，移除工具列裝飾圖示，筆數靠左，新增筆記與刪除依序靠右並排，間距 8px；寬度不足時自然換列。按鈕字級 11px、最小高度 34px，新增使用既有金色，刪除保留暗紅色並為取消刪除文字預留寬度。工具列下方 8px 接筆記列表，卡片沿用共用 panel 的邊框規則，以局部框線 token 選用較暗的 tertiary 金色，日期使用既有次要文字色。筆記摘要上下內距 5px，點選後進入獨立編輯頁。寫入筆記沿用 `primary-action branded-explore-action`，字級 14px、正常字級下高度 36px，允許放大文字時增加高度。標題卡背景、空列表圖示、帳號隔離與新增、儲存、選取刪除及確認流程維持原樣。不要再增加重複標題或卡片尾端的筆記刪除入口。
