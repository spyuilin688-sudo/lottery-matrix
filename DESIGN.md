...
歷史開獎頁的今彩539與天天樂維持既有曆週分卡、期數與「期數｜日期｜開獎號碼」三欄結構；`src/draw-history-readability.css` 只負責這兩個五球彩種的歷史頁可讀性，資料列維持 59px，五球間距調整為 `clamp(6px, 2.4vw, 10px)`；表頭底線、資料列底線與前兩欄直向分隔線分別提高至 `rgba(212, 169, 83, .48/.36/.32)`，只加強既有分隔線辨識度，不新增結構。`src/number-ball.css` 的歷史頁五球尺寸仍為 `clamp(23px, 6.8vw, 26px)`、號碼字級仍為 `clamp(12px, 3.4vw, 13.5px)`。Matrix Explore、大樂透與六合彩維持既有尺寸與間距；資料、排序、日期、篩選、API 與互動不變。
