...
「我的」頁的目前訂閱狀態卡維持已確認 A+B 黑金框架，依 2026-09-25 使用者確認的 A 方案，移除資訊區獨立圓角底板與實色背景。2026-09-25 最新出圖確認：訂閱資訊區先以左右 10px margin 抵銷框圖延伸，再套用左 14px、右 8px 內距。皇冠位於左側，寬高由前次 14.64cqw 再放大 20% 至 17.568cqw；皇冠佔位右側增加 10px 文字間距；右側剩餘空間按 9:11 分配給方案與到期資訊，兩組文字各自靠左，分隔線在兩組文字間的空白置中並延伸至資訊區底部，兩組文字與欄寬位置維持不變；到期文字保留原本距欄左 15px 的位置（舊有 1px 邊線加 14px 內距），分隔線從欄邊往左 14px，皇冠與文字組垂直置中。標題與資訊列之間的基準間距為 2px，資訊區底部不加內距，資訊列下緣以 2.3cqw 流動內收，最後一行文字與訂閱入口保留至少 8px；卡片隨內容自然縮短，不鎖定高度或縮小字級。各方案背景以低透明度淡金橢圓漸層向四周透明淡出，直接融入原框內背景，終生方案不再使用右上局部亮光。五款皇冠沿用原素材與比例，統一以 saturate(.72) brightness(.88) 降低飽和度與亮度。舊素材的中央垂直裝飾分隔線保持移除，兩欄文字間僅由 CSS 繪製一條細分隔線；購買入口隱藏時，既有 SVG 遮罩同步移除原素材內的按鈕框與箭頭，避免到期資訊與不存在的操作框重疊。所有調整直接由既有規則擁有，不追加覆寫層。免費會員、月費方案、季費方案、年費方案、終生／終身方案共用相同資訊結構、CTA 與互動流程，只依方案切換資訊區背景、五款獨立皇冠識別與方案名稱金色色階；舊皇冠由原素材遮罩移除，不與方案皇冠重疊。方案、說明、到期日及剩餘天數仍使用既有資料來源與邏輯；付費方案說明維持單行「Matrix Pro 權限」，免費會員仍顯示「核心功能體驗」。卡片高度依內容自然延伸，既有 SVG 上下分片維持比例，中段直線框隨內容延伸；最後一行與底部訂閱入口保留至少 8px，入口維持至少 44px 觸控高度。320–430px 保持不超框、不重疊及無水平溢位。

### Member detail card backgrounds — 2026-09-25

訂閱方案與收費標準的月／季／年方案卡共用低對比淡金斜紋，僅在右側及右下逐漸呈現，價格、天數、權限文字與既有功能圖示保持清楚；管理訂閱／續訂方案卡不套用。關於樂彩 Matrix 卡片右下及「我的推薦碼＋推薦成功人數」摘要區使用淡 M 輪廓，向正文方向淡出；推薦碼輸入、啟動碼與規則區不套用。背景由 `src/feature-pages.css` 的局部偽元素單獨擁有，金色沿用 `--pwa-frame-secondary`，斜紋／M 的 opacity 分別為 `.20`／`.06`。M 維持等比例並受容器尺寸限制；裝飾不參與排版、不攔截點擊，不新增圖片、文字、Logo、外框、光暈或動畫。A+B 主視覺、Header、Bottom Navigation、現有卡片尺寸、文字、按鈕、資料及所有流程不變。
...
2026-09-26 首次提示改為使用者提供的授權條款與免責聲明。共用 AppDialog 使用 `first-visit-consent` 變體：手機寬度隨視窗縮放、最大 360px；正文 13px 靠左、分段，內容過長時在卡片內捲動；單一同意按鈕保留 44px 觸控高度，卡片外沿用既有背景模糊。2026-09-08 的自訂條件介面規格保留：彩種下方單列「探索期數：十三期 | 探索範圍：完整範圍」，兩側等寬讓分隔符固定於中央；預設／自訂狀態移至一碼條件標題右側；版路複選沿用 `.segmented-static` 金色選取樣式，原生 checkbox 覆蓋整格且保留鍵盤操作。後續整體整理採收合群組：區段標題14px/600、群組名稱13px/600暖白、摘要11px/400灰白；欄位標籤12px/400，值13px/500，欄位及版路按鍵30px、列距6px。群組收合標題最少52px，內容左右10px；新增條件28px、新增群組30px、儲存與重置34px；底部操作列以共用導覽清除高度加8px留距，避免遮住操作；取消卡內新增按鍵的虛線框，範圍端點直接並排，重複摘要與端點標籤僅保留供輔助技術讀取。原有自訂條件、儲存與重置流程不變。
...
牌單同彩種更新失敗時保留最後成功資料，顯示「更新失敗，目前顯示上次資料」；首次讀取失敗沿用原錯誤提示，切換彩種清除先前內容。同彩種進行中的讀取共用一次請求，新發布訊號及訂閱就緒後必要的補讀依序處理，不增加固定輪詢。樣式與下載流程維持既有契約。
