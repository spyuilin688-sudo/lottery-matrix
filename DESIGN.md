...
「我的」頁的目前訂閱狀態卡維持已確認 A+B 黑金框架，依 2026-09-25 使用者確認的 A 方案，移除資訊區獨立圓角底板與實色背景。2026-09-25 最新出圖確認：訂閱資訊區先以左右 10px margin 抵銷框圖延伸，再套用左 14px、右 8px 內距。皇冠位於左側，寬高由前次 14.64cqw 再放大 20% 至 17.568cqw；右側剩餘空間按 2:3 分配給方案與到期資訊，方案文字靠近皇冠，兩組文字各自靠左，分隔線位於兩欄交界處且延伸至資訊區底部，皇冠與文字組垂直置中。資訊區底部不加內距，最後一行至訂閱入口保留既有 8px 間距，卡片隨內容自然縮短，不鎖定高度或縮小字級。各方案背景以低透明度淡金橢圓漸層向四周透明淡出，直接融入原框內背景，終生方案不再使用右上局部亮光。五款皇冠沿用原素材與比例，統一以 saturate(.72) brightness(.88) 降低飽和度與亮度。舊素材的中央垂直裝飾分隔線保持移除，兩欄文字間僅由 CSS 繪製一條細分隔線；購買入口隱藏時，既有 SVG 遮罩同步移除原素材內的按鈕框與箭頭，避免到期資訊與不存在的操作框重疊。所有調整直接由既有規則擁有，不追加覆寫層。免費會員、月費方案、季費方案、年費方案、終生／終身方案共用相同資訊結構、CTA 與互動流程，只依方案切換資訊區背景、五款獨立皇冠識別與方案名稱金色色階；舊皇冠由原素材遮罩移除，不與方案皇冠重疊。方案、說明、到期日及剩餘天數仍使用既有資料來源與邏輯；付費方案說明維持單行「Matrix Pro 權限」，免費會員仍顯示「核心功能體驗」。卡片高度依內容自然延伸，既有 SVG 上下分片維持比例，中段直線框隨內容延伸；最後一行與底部訂閱入口保留至少 8px，入口維持至少 44px 觸控高度。320–430px 保持不超框、不重疊及無水平溢位。
