...
def build_explore_batch(
    *, lottery: str, draw_period: str, history: list[dict[str, Any]], position_count: int,
    start: int, limit: int, existing: dict[str, Any] | None,
    ...
    artifact = deepcopy(existing) if existing else {
        'lottery': lottery, 'drawPeriod': draw_period, 'items': [], 'validationById': {},
    }
    for unit in units[cursor:stop]:
        raw = runner(unit, history)
        if append_result is not None:
            append_result(artifact, unit, raw)
    return {'artifact': artifact, 'cursor': stop, 'total': len(units), 'complete': stop >= len(units)}
