...
INVALID_ONE_CODE_TIE = "準4+鎖定1碼最終必須恰好1條最長共同規則"
...
def candidate_value(road: RoadType, base: int, target: int, maximum: int) -> int:
    if road is RoadType.SUM:
        return base + target
    # +0 is valid for both Add and Drag.
...
def evaluate_one_code(groups: Iterable[Iterable[int]]) -> StreakDecision:
    ...
    if highest >= 8:
        return StreakDecision(
            False, highest, longest_rules, INVALID_ONE_CODE_MAXIMUM, matched
        )
    if len(longest_rules) > 2:
        return StreakDecision(
            False, highest, longest_rules, INVALID_MORE_THAN_TWO_LONGEST, matched
        )
    if len(longest_rules) != 1:
        return StreakDecision(
            False, highest, longest_rules, INVALID_ONE_CODE_TIE, matched
        )
...
def _record_pair_scores(
    scores: dict[tuple[int, int], int],
    first: int,
    seconds: Iterable[int],
    streak: int,
) -> None:
    """Record bounded final/expired states only; never build a cross-group Cartesian product."""
...
def _two_code_state_scores(
    groups: tuple[frozenset[int], ...],
) -> dict[tuple[int, int], int]:
    """Canonical lock2 state machine.

    Anchors originate from B/C only. For each first rule we maintain one bounded
    second-candidate set. A group containing the first rule keeps the second pool
    alive unchanged; a group missing the first rule intersects the pool. Thus a
    retained second rule may be confirmed much later (D/E/F/...), including in a
    group that also contains the first rule. No B×C×D... pair product is built.
    """
    ...
    bounded = groups[:12]
    b, c = bounded[0], bounded[1]
    anchors = b.union(c)
    scores: dict[tuple[int, int], int] = {}

    for first in anchors:
        # The second rule must originate from the original B/C candidate pool.
        second_pool = set(b.union(c))
        second_pool.discard(first)

        # Any initial group not covered by first must be covered by second.
        if first not in b:
            second_pool.intersection_update(b)
        if first not in c:
            second_pool.intersection_update(c)

        if not second_pool:
            continue

        # B and C are already covered by {first, second_pool}.
        for group_index, group in enumerate(bounded[2:], start=2):
            if first in group:
                # Do not require the second rule to appear here. It can be
                # confirmed later while the first rule keeps extending.
                continue

            before = set(second_pool)
            second_pool.intersection_update(group)
            expired = before.difference(second_pool)
            if expired:
                _record_pair_scores(scores, first, expired, group_index)

            if not second_pool:
                break
        else:
            _record_pair_scores(scores, first, second_pool, len(bounded))
            continue
...
def evaluate_two_code(groups: Iterable[Iterable[int]]) -> StreakDecision:
    ...
    scores = _two_code_state_scores(normalized)
    if not scores:
    ...
    pair = tuple(sorted(longest_rules))
    if len(pair) != 2:
        return StreakDecision(False, highest, longest_rules, reason="準5+最終必須恰好2條不同規則")
...
class ExploreEngineContext:
    """One indexed newest-first history shared by all canonical Explore roads."""
    ...
    def official_at(self, draw_index: int) -> tuple[int, ...]:
        return _complete_int_tuple(self._draws[draw_index].get("numbers"), self.position_count)
    ...
    def range_cells(
        ...
        # Build Full once. Standard is a filter over these cells.
    ...
    def _target_map(
        ...
        # Formula values are the rule identity. Multiple hit targets stay attached
        # to one rule instead of becoming duplicate rules.
...
class ExploreEngineSession:
    ...
    contexts: tuple[ExploreEngineContext, ...]
    indexed_units: tuple[tuple[ExploreEngineContext, SourceUnit], ...]
    ...
    def build(
        ...
        contexts = tuple(
            ExploreEngineContext.build(
                lottery,
                order,
                draw_order_history(lottery, history) if order == DRAW_ORDER else history,
            )
            for order in orders
        )
        indexed_units = tuple(
...
def _rule_payload(road: RoadType, value: int) -> dict[str, object]:
    return {"value": value, "display": _rule_display(road, value), "algorithmType": road.value}
    ...
    for rules in decision.rule_sets:
            ...
            "ruleSets": [
                {
                    ...
                    "historicalValidation": _historical_validation(
                        context, groups, road, ordered_rules, decision.highest_streak
                    ),
                }
            ],
        }
        ...
        if existing is not None:
            if existing != validation:
                raise ValueError("EXPLORE_ENGINE_RESULT_CONFLICT")
            continue
...
def _evaluate_groups(
    ...
    # Decisions are coordinate-local. Never aggregate same-longest rules across
    # different reference offsets/positions.
...
def _range_bundles(
    ...
    # Full history is indexed/searchable. We only need up to 12 same-lock
    # occurrences to prove the legal maximum has been exceeded.
...
def _groups_for_cell(
    ...
    for occurrence, result_index, by_coordinate in bundles:
        ...
        if (
            explore_range == "標準範圍"
            and candidate.cell.scope_class is ScopeClass.FULL_ONLY
        ):
            break
        groups.append(
            RoadGroup(
        ...
        groups.append(
            RoadGroup(
                ...
                candidate_targets=context.drag_candidate_targets(
                    occurrence, unit.prediction_distance
                ),
            )
        )
        if len(groups) >= limit:
...
def _run_engine_unit(
        ...
        for source_cell in source_cells:
            for explore_range in EXPLORE_RANGES:
                ...
                for road in range_roads:
                    groups = _groups_for_cell(
                        bundles, source_cell, road, explore_range
                    )
                    _evaluate_groups(
                        artifact,
                        context,
    ...
    if RoadType.DRAG in road_types:
        # Drag has one canonical calculation because its evidence never changes
        # between Standard and Full. RPC scope filtering exposes this row to both.
...
def run_explore_engine_batch(
    ...
    *,
    ...
    if session is None:
        ...
        raise ValueError("EXPLORE_ENGINE_SESSION_MISMATCH")
