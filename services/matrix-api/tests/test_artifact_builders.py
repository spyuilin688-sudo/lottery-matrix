...
def test_explore_builder_creates_canonical_detached_rows() -> None:
    ...
    assert artifact["tianyanSources"] == [{"id": "candidate-a"}]


def test_explore_batch_builder_needs_only_start_and_limit() -> None:
    builders = create_artifact_builders(explore_runner=lambda *_: {"results": [], "tianyanSources": []})
    context = {
    ...
    assert result["artifact"]["tianyanSources"] == []
...
def test_concrete_status_builder_uses_completed_explore_artifact() -> None:
    ...
    context = {
        ...
        "artifacts": {"explore": {"lottery": "今彩539", "drawPeriod": "123", "items": [], "validationById": {}, "tianyanSources": []}},
    }
