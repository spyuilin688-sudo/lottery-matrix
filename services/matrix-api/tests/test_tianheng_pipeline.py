    ...
    def table(self, name):
        ...
        class Query:
            ...
            def execute(self):
                ...
                return SimpleNamespace(data=[row for row in client.rows.values()
                    if all(row[key] == value for key, value in self.filters.items())][:1])
...
@pytest.mark.parametrize("existing", ["neither", "explore", "tianheng"])
