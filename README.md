...
現行可執行架構為 React＋TypeScript＋Vite。`src/` 是主要應用程式來源；`worker/` 與 `.openai/hosting.json` 提供既有 Sites 靜態前端封裝。已移除未被正式入口引用的 Next／D1 範本；歷史版本可由 Git 查閱，Supabase migrations 與既有 Sites 封裝保留。
